...
namespace MyriadCreation.Simulation.Systems
{
 ...
 // 战争爆发：自动撤销双方的军事通行权
            if (_realms.TryGetValue(attackerId, out var atkRealm))
                MyriadCreation.Simulation.Warfare.MovementControlSystem.OnWarDeclared(atkRealm, defenderId);
            if (_realms.TryGetValue(defenderId, out var defRealm))
                MyriadCreation.Simulation.Warfare.MovementControlSystem.OnWarDeclared(defRealm, attackerId);
 ...
 // 战争爆发：自动撤销双方的军事通行权
            if (_realms.TryGetValue(attackerId, out var atkRealm2))
                MyriadCreation.Simulation.Warfare.MovementControlSystem.OnWarDeclared(atkRealm2, defenderId);
            if (_realms.TryGetValue(defenderId, out var defRealm2))
                MyriadCreation.Simulation.Warfare.MovementControlSystem.OnWarDeclared(defRealm2, attackerId);
 ...
 // ===== 内部辅助 =====
        ...
        {
 // 遍历全部外交关系：与防御方有 mutualDefense 盟约的第三方加入对攻击方宣战 // （修复：原实现要求 rel.isAtWar 才处理，导致防御同盟义务永不触发）
