...
namespace CivilizationEvolution.Simulation.Characters
{
 ...
 // 疯王（NPD 式统治风格——未必有病——卡利古拉/尼禄：自恋傲慢+偏执+ // 喜怒无常[高报复+低理性]+任性妄为[高大胆+低荣誉]——统治 10 年+行为证据）
 ...
 // 圣君（在位圣明——尧舜式——德行×治理双极致：评价≥卓越[750]+ // 悲悯荣誉双高[60+]+无饥荒少叛乱——王级苛刻——与圣者[死后封圣]区分）
 ...
 // 无冠者（实权无冕——非在位君主但达成君主级成就——宫相式—— // 身份型综合先于行为型绰号判定）
 ...
 // 年轻者（青年路易二世式讽刺：幼年即位[<16]是事实——不够老练才是 // 核心语义——被摄政架空或决策反复[低理性]——幼稚暗讽）
                if (isRuler && rec.youngAccession &&
                    (rec.ruledUnderRegency || c.rationality < -20f))
                    return GrantEpithet(c, "年轻者") ? "年轻者" : "";
 // 护国公（摄政护主——非君主但掌实权+名正言顺[为幼主/空位摄政—— // 区别于无冠者[僭越式实权]——克伦威尔式）
 ...
 // 背信者（背盟撕约——faithChanges 反向[外交背盟计数近似]—— // 用 schemesSucceeded 高+honor 低=言而无信者）
 ...
 // ===== ④ 外貌/身体型（bodyMarks——事件/伤病写入——任何身份—— // 成对体系：黑/白——统治者=王/继承人或贵族=王子——其余身份=者） =====
 ...
 // 黑/白=军事贵族成对体系（甲色/发色形象——按身份阶梯）： // 骑士→黑/白骑士；继承人/贵族→黑/白王子；君主→黑/白王
                        case "黑色":
