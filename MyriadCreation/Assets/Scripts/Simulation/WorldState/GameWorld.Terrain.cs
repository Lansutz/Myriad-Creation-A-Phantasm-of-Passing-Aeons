...
namespace MyriadCreation.Simulation.WorldState
{
 ...
 /// <summary>全部生成（地形+气候+水文，编辑器一键生成）</summary>
        ...
        {
 // 省份密度随地图尺寸动态调整：大地图每省地块更多，避免省份数量爆炸 // 公式：Max(48, sqrt(总地块) * 0.3) // 128×64(8192)→48地块/省≈170省；512×256(131072)→109≈1200省；1920×1080(2073600)→432≈4800省（对齐参考项目5226省）
 ...
 /// <summary>生成子地块/Burg（省份内定居点：城市/港口/要塞/村庄）</summary>
        private void GenerateBurgs(int seed)
        {
            ...
            var (genAnchors, genSettlements) = generator.Generate();
            anchors = genAnchors;
            burgs = genSettlements;
