...
namespace CivilizationEvolution.Simulation.WorldState
{
 ...
 // 政体变迁动力学需要革新树（可行性约束）与编年史（记录节点）
            ...
            _settlementSimulationSystem = new SettlementSimulationSystem(
                burgs,
                tiles,
                mapWidth,
                mapHeight,
            ...
            _aiIntentExecutor = new AIIntentExecutor(_simulationCommands);
 ...
 /// <summary>初始化宗教运行时（每教统一个 FaithSystem——热忱/美德罪行/领袖）</summary>
        ...
        {
            _faithSystems.Clear();
            foreach (var kv in ReligionCatalog.All)
            {
                var def = kv.Value;
                if (def.nodeType != ReligionNodeType.Succession) continue; // 只教统有运行时
