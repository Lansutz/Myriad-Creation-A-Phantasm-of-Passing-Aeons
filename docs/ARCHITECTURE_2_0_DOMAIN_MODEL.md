...
## Architecture 2.0 的拆分尺度与平衡原则

本项目学习 CK3 的复杂模拟架构，但“更好”不等于“拆得更多”。每一个新抽象都必须解决真实的责任、依赖、性能、Mod 扩展、测试、复用、生命周期或调试问题。

最终同时优化以下维度：

1. **性能与优化**：大地图、大量人口/角色/政权下保持可控的 Tick 成本；优先考虑批处理、Dirty、缓存和派生状态。
2. **Mod 修改便捷性**：内容定义、规则、条件、效果和 AI 偏好应有清晰扩展点，同时保持类型安全。
3. **逻辑清晰**：明确状态所有权、读取者、写入者、生命周期和因果链。
4. **模拟表现力**：支持复杂因果关系，而不是把所有机制压缩成脚本式事件。
5. **可维护与可测试**：领域可以独立验证、替换和重构，避免 GameWorld/God Object。
6. **内容生产效率**：新增内容尽可能复用既有 Definition / Rule / Action / Effect / Query，而不是复制一套系统。
7. **可预测与可调试**：能够追踪“为什么发生、谁决定、谁执行、改变了什么”。
8. **模拟成本意识**：每个每日/每月逻辑都必须考虑执行频率、对象规模、缓存与失效策略。

### 反过度架构规则

概念边界不等于必须存在一个独立类。只有当边界具有实际工程价值时才实体化。

允许：
`AI → Intent → Domain Command → Domain State → Event`

不鼓励：
`Intent → Action → Process → Command → Handler → Service → System`
其中中间层只做参数转发。

**目标不是类数量最大化，而是复杂度不变时降低错误耦合。**
