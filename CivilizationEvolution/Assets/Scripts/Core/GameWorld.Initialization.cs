...
{
 /// GameWorld.Initialization —— 世界初始化与数据定义（物资/兵种/种族/文化/政权/信仰）（partial class，与 GameWorld.cs 共享字段与子系统）    public partial class GameWorld
    {

 /// <summary>初始化世界（StartNewGame 设好尺寸后显式调用）</summary>        public void InitializeWorld()
        {
 // 初始化生成管线（在所有子系统之前，幂等）            Pipeline ??= new GenerationPipeline(this);
 // 幂等（重复 StartNewGame 安全——重新建 tiles）            if (startYear < 0) { startYear = currentYear; startDay = currentDay; }
 // 内容注册表幂等初始化（与 Bootstrap.Awake 无执行顺序依赖）            if (!ContentRegistry.IsInitialized)
                ContentRegistry.Initialize();

 // 配置资产：拖入了SO则Instantiate运行时副本（避免污染资产），否则创建默认实例            if (config == null)
            {
 ...
 // 阶层出现事件订阅（革新完成→检测解锁阶层→编年史——查漏补缺接线）            _innovationTree.OnInnovationCompleted += OnInnovationCompletedHandler;
            _chronicle = new Chronicle(); // 编年史（世界大事日志）
 // 政体变迁动力学需要革新树（可行性约束）与编年史（记录节点）            _regimeDynamics = new RegimeChangeDynamics(_innovationTree, _chronicle);
            _diplomacyManager.Chronicle = _chronicle;
            _aiManager = new AIManager();
        }


 /// <summary>读档后重建子系统（引用类型无法序列化）</summary>        public void ReinitializeSubsystems()
        {
            InitializeSubsystems();
 ...
 // ===== 初始化辅助 =====        private void InitializeGoodsDefs()
        {
        ...
        {
 // 步兵（兵种×革新：重装需铁制武器、精锐需炼钢、弩手需弩；物资：武器/盔甲）            AddUnitDef(100, "轻装步兵", GameEnums.UnitCategory.Infantry, 1, 10f, 0f, 8f, 60f, 2f, 1f, 50f, 1f);
 ...
 // 骑兵（轻骑需骑乘术、重骑需马镫、精锐需重装骑兵；物资：武器+盔甲+马）            AddUnitDef(200, "轻骑兵", GameEnums.UnitCategory.Cavalry, 1, 12f, 5f, 8f, 65f, 4f, 1.5f, 60f, 1f, 923);
 ...
 // 水军（桨帆需桨帆船、战舰需克拉克、撞角需撞角战术、远洋贸易船需远洋贸易）            AddUnitDef(300, "桨帆船", GameEnums.UnitCategory.Navy, 1, 15f, 10f, 10f, 60f, 3f, 2f, 80f, 1f, 402);
 ...
 // 步兵：武器/盔甲            SetUnitRecruitCosts(101, (70, 1.2f), (71, 1f));
            SetUnitRecruitCosts(102, (70, 1.5f), (71, 1.5f));
 // 骑兵：武器+盔甲+马            SetUnitRecruitCosts(200, (70, 1f), (20, 1f));
 ...
 // 船只：原木/加工木材 + 铁矿/棉花            SetUnitRecruitCosts(300, (30, 2f), (31, 1f));
 ...
 /// <summary>设置兵种招募物资（经济系统 goods 对接——goodsId → 数量）</summary>        private void SetUnitRecruitCosts(int unitId, params (int goodsId, float amount)[] costs)
        {
 ...
 // 开局启动粮储（物资 id 0 = 粮食）：经济产出尚未运转前，避免有人口的地块瞬间全判饥荒                center.inventory[0] = 1000f;
 ...
 // 当前仅人类            races[0] = new RaceData
        ...
        {
 // 找到陆地地块分配给初始政权            var landTiles = new List<int>();
            for (int i = 0; i < tiles.Length; i++)
            {
                if (tiles[i].isLand)
                    landTiles.Add(i);
 ...
 // 创建3个初始政权，各占1/3陆地            int perRealm = landTiles.Count / 3;
            for (int r = 0; r < 3; r++)
            {
 ...
 // 给每块地分配初始人口（，全部人族）                    if (tiles[tileIdx].populationBlocks.Count == 0)
                    {
 ...
 // 为每个政权创建AI控制器                _aiManager.CreateController(r);
            }

 // 为每个政权创建统治者与配偶（角色系统与生育机制的源头）            _characterManager.CreateInitialRulers(realms, currentYear);

 // 外交关系懒加载：首次GetRelation时自动创建        }


 /// 内容注册表覆盖（数据驱动优先） /// ContentRegistry 已加载的 Base/Mods 内容按 id 覆盖内置默认； /// 与 ContentRegistry 的 "Mods 同名 Id 覆盖 Base" 语义一致。 /// 注意：内容包 cultureId/raceId 须 > 0（ContentRegistry 的约定），内置默认为 0,1,2， /// 内容 id 与内置重叠时以内容为准。        private void ApplyContentOverrides()
        {
 ...
 // 文化地图颜色自动分配（未配置的文化按 id 取色）            foreach (var kv in cultures)
 ...
 /// <summary>初始化宗教运行时（每教统一个 FaithSystem——热忱/美德罪行/领袖）</summary>        private void InitializeFaithSystems()
        {
            _faithSystems.Clear();
