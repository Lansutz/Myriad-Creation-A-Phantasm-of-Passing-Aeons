...
{
 /// GameWorld.Simulation —— 主循环模拟Tick（人口/政治/社会/传教/信仰/时间推进）（partial class，与 GameWorld.cs 共享字段与子系统）    public partial class GameWorld
    {

 /// 人口Tick：自然增长、满意度、迁徙——Parallel.For 多核并行 /// （优化地块每日热循环——每 tile 独立无 /// 共享写——事件入队改并发收集[ConcurrentQueue]后主线程统一入队—— /// 读共享字典/贸易中心安全[模拟单线程 Tick 内独占并行段]）        private void PopulationTick()
        {
 ...
 // 人口承载约束（超载抑制增长——地理/仓储/贸易决定上限）                        float capacity = CarryingCapacitySystem.CalculateCarryingCapacity(tiles[i], tradeCenters);
                        float overload = CarryingCapacitySystem.GetOverloadRatio(tiles[i], capacity);
                        if (overload > 1f)
                        {
 // 超载 10% 起线性抑制，超载 200% 完全停止                            float suppression = Mathf.Clamp01((overload - 1.1f) / 0.9f);
 ...
 // 并发收集事件统一入队（EnqueueEvent 主线程专用——顺序无关）            while (parallelEvents.TryDequeue(out var ev))
                EnqueueEvent(ev);
        }


 /// <summary>计算人口块满意度变化</summary>        private float CalculateSatisfactionDelta(int tileIndex, PopulationBlock pb)
        {
            float delta = 0f;
 ...
 /// <summary>政治Tick（兼容层：税收结算+稳定值）</summary>        private void PoliticsTick()
        {
 // 社会分化按固定间隔推进（对所有政权同步），避免每天搬运人口            _differentiationTimer += daysPerTick;
            bool doDifferentiation = _differentiationTimer >= DifferentiationIntervalDays;
            if (doDifferentiation) _differentiationTimer = 0f;

 // 税收结算（政权循环——轻）            foreach (var realm in realms.Values)
            {
                float taxIncome = _economyManager.SettleTaxes(realm.realmId);
                realm.treasury += taxIncome;
            }

 // 稳定值收敛（优化全图扫=N×tiles—— // 改单次全扫 O(tiles)——每 tile 按其所属政权集权度收敛）            {
 // 政权集权度缓存（每 tick 一次构建——避免循环内 realms 查）                var centralCache = new Dictionary<int, float>(realms.Count);
 ...
 // 领地索引（优化：单次全扫构建 ownerRealmId→地块——各政权社会 // 评估复用——替代每政权全扫[N×tiles]）            var realmTilesIndex = new Dictionary<int, List<int>>();
            for (int i = 0; i < tiles.Length; i++)
            {
                if (!tiles[i].exists) continue;
                int owner = tiles[i].ownerRealmId;
 ...
 // 社会-派系-政体变迁脉冲（每政权——内部评估——用领地索引免全扫）            foreach (var realm in realms.Values)
            {
 ...
 /// 社会-派系-政体变迁脉冲：情境采集 → 阶层需求满足度 → 政治能量 → 派系组织化 → 关键节点博弈。 /// 阶层好感在此由真实需求满足度驱动（替代旧的机械回归中性值）。        private void SocietyPulse(RealmData realm, bool doDifferentiation,
 ...
 // 先推进社会分工（人口在阶层间缓慢、守恒转移），再统计社会画像，保证派系看到的是最新阶层结构            if (doDifferentiation)
 ...
 /// 行政区划树生成（政权辖境→AdminDivisionSystem——治理模式定层数： /// 分封 4 层/郡县=行政容量弹性——官僚头衔随层绑定——批4）        private void EnsureAdminDivisions()
        {
            for (int i = 0; i < realms.Count; i++)
            {
                var realm = realms[i];
                if (realm == null || realm.adminDivisions.Count > 0) continue;
 // 辖境=核心+占领地块                var territory = new HashSet<int>(realm.coreTiles);
 ...
 /// 政权传教（15 天一次——国教政权对其境内异教主流块传教—— /// 成功率=冲突度[同根 0.6/异教 0.2]——ConvertTile——限频省扫描）        private void MissionaryTick()
        {
 ...
 // 政权→国教映射（一次构建）            var realmFaith = new Dictionary<int, int>();
            foreach (var r in realms.Values)
 ...
 // 本地主流已是国教→跳过                int localFaith = Politics.PopulationStats.GetDominantFaith(tile);
                if (localFaith == stateFaith) continue;
 // 传教（成功率=冲突度）                float chance = Culture.MissionarySystem.CalculateSuccessChance(tile, stateFaith,
 ...
 /// 官职任命（Governor 等 6 官职——空缺补任——从政权角色中选非统治者者—— /// OfficeTitle 官职称号系统的持有者数据源）        private void EnsureOfficeHolders()
        {
 ...
 // 空缺——从政权角色选（非统治者——优先 Noble/Military）                    var candidates = _characterManager.GetCharactersByRealm(realm.realmId);
 ...
 /// <summary>时间推进</summary>        private void AdvanceTime()
        {
 ...
 /// <summary>信仰热忱每日更新（30 天限频——长期和平冷却 -10/年——圣地丢失由 /// 圣地系统检测；异教冲突由宣战处 AddFervor）</summary>        private void UpdateFaithFervor(int currentDay)
        {
            if (currentDay - _faithFervorDay < FaithFervorInterval) return;
            _faithFervorDay = currentDay;
            foreach (var faith in _faithSystems)
            {
 // 长期和平冷却（约每年一次——每 360 天 -10）                if (faith.fervor > 10f && currentDay % 360 == 0)
                    faith.AddFervor(-10f);

 // 圣地丢失检测（己方圣地被异教政权控制 → +50——"收复失地"狂热—— // 十字军启动器；一次性标记防重复刷）                CheckHolySiteLoss(faith);
            }
        }


 /// <summary>圣地丢失检测（圣地地块被非本教统国教政权控制 → 热忱+50）</summary>        private void CheckHolySiteLoss(FaithSystem faith)
        {
 ...
 // 被异教控制（有国教且不同信仰）→ 丢失                if (ownerFaith >= 0 && ownerFaith != faith.faithId)
                {
                    faith.AddFervor(50f);
 ...
 /// <summary>大圣战结算检查（关联 WarState 结束→OnLinkedWarEnded—— /// 圣战方胜→受益人谈判[继承法线外者]——土地归受益人[简化：目标地块转移]）</summary>        private void CheckGreatHolyWarSettlements()
        {
            ...
            foreach (var ghw in active)
            {
                ...
                {
 // 目标地块归受益人（新政权/附庸——简化：地块归属转移+编年史）                    tiles[ghw.targetTile].ownerRealmId = ghw.callerRealmId;
