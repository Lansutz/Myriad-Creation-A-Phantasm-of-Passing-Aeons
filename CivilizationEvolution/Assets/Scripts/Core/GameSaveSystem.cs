...
{
 /// 完整游戏存档系统——保存/加载核心游戏状态（地图、时间、政权、编年史）。 /// 大战略游戏完整存档，不只是地图。 /// 角色、军队、战争、外交等复杂数据后续逐步扩展。    public class GameSaveSystem
    {
        private readonly GameWorld _world;

 // 存档目录        private static string SaveDirectory => Path.Combine(Application.persistentDataPath, "GameSaves");
 ...
 // ===== 保存 =====
 /// <summary>保存完整游戏状态到指定文件名（JSON格式）</summary>        public string SaveGame(string fileName, string saveName = null)
        {
            if (_world == null)
            {
 ...
 // 1. 保存地图数据                SaveMapData(saveData);

 // 2. 保存政权基本信息                SaveRealms(saveData);

 // 3. 保存编年史                SaveChronicle(saveData);

 // 序列化到JSON                string json = JsonUtility.ToJson(saveData, true);
 ...
 // 省份            if (_world.provinces != null && _world.provinces.Count > 0)
            {
 ...
 // 聚落            if (_world.burgs != null && _world.burgs.Count > 0)
            {
 ...
 // 控制的地块                var controlled = new List<int>();
 ...
 // ===== 加载 =====
 /// <summary>从指定文件名加载完整游戏状态（JSON格式）</summary>        public bool LoadGame(string fileName)
        {
            string path = Path.Combine(SaveDirectory, fileName + ".json");
            if (!File.Exists(path))
            {
 ...
 // 1. 恢复基础数据                _world.mapWidth = saveData.mapWidth;
 ...
 // 2. 恢复地图数据                LoadMapData(saveData);

 // 3. 恢复政权基本信息                LoadRealms(saveData);

 // 4. 恢复编年史                LoadChronicle(saveData);
 ...
 // 验证尺寸            if (data.mapWidth * data.mapHeight != data.tiles.Length)
            {
 ...
 // 恢复聚落            _world.burgs = new Dictionary<int, BurgData>();
            if (data.burgs != null)
            {
 ...
 // 编年史恢复（通过 Add 方法重新添加）            foreach (var e in data.chronicleEntries)
            {
 ...
 // ===== 工具方法 =====
 /// <summary>获取所有存档文件名</summary>        public static string[] GetAllSaves()
        {
            if (!Directory.Exists(SaveDirectory))
                return new string[0];
 ...
 /// <summary>删除指定存档</summary>        public static bool DeleteSave(string fileName)
        {
            string path = Path.Combine(SaveDirectory, fileName + ".json");
            if (File.Exists(path))
            {
