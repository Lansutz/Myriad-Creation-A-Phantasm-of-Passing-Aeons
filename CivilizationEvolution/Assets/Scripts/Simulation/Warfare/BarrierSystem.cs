...
namespace CivilizationEvolution.Military
{
 ...
 /// - 堡垒（Fort）：区域控制，不直接阻挡通行，但敌对时经过有损耗和速度影响，己方给补给支援
    ...
    {
 // ===== 坡度/海拔阈值（综合判定可通行性）===== /// <summary>极难通行海拔阈值（高于此海拔且坡度足够陡峭才不可通行）</summary>
        public const float ImpassableElevation = 0.85f;

 /// <summary>极难通行坡度阈值（度）——需同时满足高海拔</summary>
 ...
 // ===== 关隘建造条件 ===== /// <summary>关隘所需的通道狭窄度（两侧极难通行/高成本地块的最小数量）</summary>
 ...
 // 综合海拔+坡度决定可通行性 // 只有高海拔(>0.85)且极陡峭(>50°)才不可通行（高山绝壁/雪线以上陡峭山峰） // 山脉中间的山口（高海拔但坡度较低）可以通行 // 低海拔的陡坡（峡谷峭壁）一般也有小径可通行
 ...
 /// 平原=1.0，山脉山口=2-4，高山绝壁=极难通行(成本50)
 ...
 // ===== 海拔×坡度交互（山脉山口：高海拔但坡度适中，成本低于陡峭低海拔）===== // 高海拔+低坡度 = 高原/山口，比高海拔+高坡度容易通行
 ...
 /// 检查某地块是否为狭窄通道
 /// 判定：4方向邻居中，至少2个方向的连续3格内有极难通行/高成本地块
