...
{
 /// 战争借口（Casus Belli）——为什么开战 /// 由敌对活动、领土争端、宗教冲突等事件生成 /// 有战争借口时宣战无惩罚（或惩罚小），无借口宣战有高惩罚

 /// 战争目标（War Goal）——开战想要达到什么目的 /// 决定战争分数的计算方式和和平条约的可选条款 /// 与战争借口关联：不同借口支持不同的战争目标

 /// 和平条约条款——实际得到什么 /// 战争结束后根据战争分数和战争目标生成具体条款

 /// 战争正当性系统（War Justification System） /// 管理战争借口的生成、检查、使用，以及战争目标和和平条约的生成 /// 三层分离： /// 1. 战争借口（Casus Belli）——为什么开战（理由） /// 2. 战争目标（War Goal）——开战想要达到什么目的 /// 3. 实际条约（Peace Treaty）——战争结束后实际得到什么    public static class WarJustificationSystem
    {
 ...
 // ===== 战争借口生成 =====
 /// 劫掠事件生成战争借口（被劫掠方获得劫掠报复借口）        public static CasusBelli GenerateRaidReprisalCB(int raidedRealmId, int raiderRealmId,
 ...
 /// 边境摩擦生成战争借口（边境事件借口，可升级为边境战争）        public static CasusBelli GenerateBorderIncidentCB(int realmA, int realmB, int currentDay)
        {
 ...
 /// 领土争端生成战争借口        public static CasusBelli GenerateTerritorialDisputeCB(int claimantRealmId, int targetRealmId,
 ...
 /// 检查某政权对另一政权是否有有效战争借口        public static bool HasValidCasusBelli(List<CasusBelli> allCBs, int holderRealmId,
            int targetRealmId, int currentDay)
        {
            return GetValidCasusBelli(allCBs, holderRealmId, targetRealmId, currentDay).Count > 0;
        }

 /// 获取某政权对另一政权的所有有效战争借口        public static List<CasusBelli> GetValidCasusBelli(List<CasusBelli> allCBs, int holderRealmId,
            int targetRealmId, int currentDay)
        {
 ...
 /// 获取最强的战争借口（用于宣战惩罚计算）        public static CasusBelli GetStrongestCB(List<CasusBelli> allCBs, int holderRealmId,
            int targetRealmId, int currentDay)
        {
 ...
 // ===== 宣战惩罚计算 =====
 /// 计算宣战惩罚（根据战争借口强度） /// <returns>名声惩罚、稳定度惩罚、其他政权关系惩罚</returns>        public static (float prestigePenalty, float stabilityPenalty, float relationPenalty)
            CalculateDeclarationPenalties(CasusBelli strongestCB, bool isSurpriseAttack)
        {
            if (strongestCB == null)
            {
 // 无战争借口 = 不宣而战，高惩罚                return isSurpriseAttack ?
                    (40f, 20f, 15f) : // 不宣而战
                    (30f, 15f, 10f);   // 无借口宣战
            }

 // 有战争借口，根据正当性强度计算惩罚            float strength = strongestCB.justificationStrength;
 ...
 // ===== 战争目标生成 =====
 /// 根据战争借口生成可选战争目标        public static List<GameEnums.WarGoalType> GetSupportedWarGoals(GameEnums.CasusBelliType cbType)
        {
 ...
 /// 创建战争目标        public static WarGoal CreateWarGoal(GameEnums.WarGoalType type, int attackerId, int defenderId,
 ...
 // ===== 和平条约生成 =====
 /// 根据战争分数和战争目标生成和平条约 /// 区分战争目标（想要的）和实际条约（得到的）        public static Treaty GeneratePeaceTreaty(int winnerId, int loserId, float warScore,
 ...
 // 记录原战争目标            foreach (var goal in attackerGoals)
                treaty.originalWarGoals.Add(goal.type);

 // 根据战争分数生成条约条款            float remainingScore = warScore;
            bool goalsFullyAchieved = true;

 // 检查主要战争目标是否达成            foreach (var goal in attackerGoals)
            {
 ...
 // 生成赔款条款（基础条款）            if (remainingScore >= 10f)
            {
 ...
 // 根据战争目标生成对应条款            foreach (var goal in attackerGoals)
            {
 ...
 // 停战协定（必有）            treaty.clauses.Add(new TreatyClause
            {
 ...
 /// 根据条款类型生成具体条款        private static TreatyClause? GenerateClauseByType(TreatyClauseType type,
 ...
 // ===== 边境摩擦 vs 边境战争区分 =====
 /// 检查边境摩擦是否升级为边境战争 /// 边境摩擦是低烈度冲突（不触发战争），边境战争是有限战争（有战争借口和战争目标）        public static bool ShouldBorderSkirmishEscalate(float hostilityLevel, int raidCount,
            float relationValue, float randomRoll)
        {
 // 升级条件：敌对程度高 + 累计摩擦/劫掠次数多 + 关系差            float escalationChance = 0f;
 ...
 /// 获取战争借口描述（用于UI显示） /// 执行和平条约条款（对接现有外交系统） /// 将条约条款转化为Alliance/Subordination/SpecialBond等现有结构        public static void ExecutePeaceTreaty(Treaty treaty, DiplomaticRelation rel, int currentDay)
        {
 ...
 // 经济类条款：记录在条约中，由经济系统执行（贸易权/航行权/资源特许权）                        break;
 ...
 // 人文类条款：记录在条约中，由文化/宗教系统执行                        break;
                    default:
 // 其他条款（领土/赔款/裁军等）记录在条约中，由对应系统执行                        break;
                }
            }
