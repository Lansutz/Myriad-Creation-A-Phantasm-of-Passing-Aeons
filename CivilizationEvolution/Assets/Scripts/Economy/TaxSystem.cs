...
namespace CivilizationEvolution.Economy
{
 ...
 /// <summary>计算地块实际税收</summary>        public float CalculateTileTax(TileData tile, float baseOutput, GameEnums.SocialClass dominantClass)
        {
 ...
 // 最优税率区间：超过30%后边际收益递减            float effectiveRate = combinedRate < 0.3f
                ? combinedRate
 ...
 /// <summary>计算税率对阶层好感的影响</summary>        public float GetTaxSatisfactionImpact(GameEnums.SocialClass socialClass)
        {
