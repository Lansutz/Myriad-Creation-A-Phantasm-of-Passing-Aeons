...
{
 /// 圣人定义（崇拜对象·历史出身——一神教崇拜对象的核心形态： /// 与多神教神祇[神话出身]并列——圣人=角色升格[封圣事件]）

 /// 封圣机制（圣人=角色升格——美德/罪行系统对接）： /// 已故角色（spiritualFulfillment≥80 + 美德特质≥2）→ 封圣候选 /// → 教会批准（有宗教领袖[教宗/牧首]时）→ 成为圣人 /// → 可被选为主保圣人（个人信条/政权主保——崇拜对象池）    public static class CanonizationSystem
    {
 ...
 /// <summary>封圣候选判定（死亡+灵性满足≥80+美德≥2——历史：圣徒生前极度虔诚）</summary>        public static bool IsCanonizationCandidate(FaithSystem faith, CharacterData c)
        {
 ...
 /// 执行封圣（候选+教会批准[有领袖或教统权威]→成为圣人——加入崇拜对象池） /// 返回圣人（失败 null）        public static SaintDef Canonize(FaithSystem faith, CharacterData c, string domain)
        {
            if (!IsCanonizationCandidate(faith, c)) return null;
 // 教会批准：教统有宗教领袖（教宗/牧首/大祭司）或默认权威            if (faith.highPriestCharacterId < 0) return null;

            if (!_saintsByFaith.TryGetValue(faith.faithId, out var saints))
                _saintsByFaith[faith.faithId] = saints = new List<SaintDef>();

 // 防重复封圣            foreach (var s in saints)
 ...
 /// <summary>某教统的圣人列表（主保候选池）</summary>        public static List<SaintDef> GetSaints(int faithId)
            => _saintsByFaith.TryGetValue(faithId, out var saints) ? saints : new List<SaintDef>();

 /// <summary>重置（测试用）</summary>        public static void Reset()
        {
