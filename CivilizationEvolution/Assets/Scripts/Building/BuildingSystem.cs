...
{
 /// 建筑基建系统 /// 六大核心基建：农业、手工业、道路、城防、市场、行政    [System.Serializable]
    public class BuildingSystem
    {
        ...
        {
 // 农业基建            AddBuildingDef(100, "农田", BuildingCategory.Agriculture, 1, 100f, new Dictionary<int, float> { { 30, 20f } });
 ...
 // 手工业基建            AddBuildingDef(200, "铁匠铺", BuildingCategory.Craft, 1, 200f, new Dictionary<int, float> { { 50, 10f }, { 70, 5f } });
 ...
 // 道路基建            AddBuildingDef(300, "土路", BuildingCategory.Road, 1, 50f, new Dictionary<int, float> { { 40, 5f } });
 ...
 // 城防基建            AddBuildingDef(400, "木栅栏", BuildingCategory.Defense, 1, 100f, new Dictionary<int, float> { { 30, 20f } });
 ...
 // 市场基建            AddBuildingDef(500, "集市", BuildingCategory.Market, 1, 150f, new Dictionary<int, float> { { 30, 10f } });
 ...
 // 行政基建            AddBuildingDef(600, "村社公所", BuildingCategory.Admin, 1, 100f, new Dictionary<int, float> { { 30, 10f } });
 ...
 /// <summary>建造建筑</summary>        public bool BuildBuilding(int tileIndex, int buildingId, int realmId, RealmData realm)
        {
 ...
 // 检查资源            if (realm.treasury < def.buildCost) return false;

 // 检查建筑上限（每地块最多5个建筑）            if (!_tileBuildings.ContainsKey(tileIndex))
                _tileBuildings[tileIndex] = new List<ActiveBuilding>();
            if (_tileBuildings[tileIndex].Count >= 5) return false;

 // 扣除资源            realm.treasury -= def.buildCost;

 // 开始建造            var building = new ActiveBuilding
            {
 ...
 /// <summary>每日建筑Tick（建造进度）</summary>        public void DailyTick()
        {
 ...
 /// <summary>建筑完成时触发</summary>        private void OnBuildingComplete(ActiveBuilding building)
        {
            if (!_buildingDefs.TryGetValue(building.buildingId, out var def)) return;

 // 更新地块基建等级            int categoryIndex = (int)def.category;
 ...
 // 应用效果            ApplyBuildingEffects(building.tileIndex, def, true);

            Debug.Log($"[Building] {def.buildingName} 建造完成，地块 {building.tileIndex}");
        }

 /// <summary>应用建筑效果</summary>        private void ApplyBuildingEffects(int tileIndex, BuildingDef def, bool apply)
        {
 ...
 /// <summary>拆除建筑</summary>        public bool DemolishBuilding(int tileIndex, int buildingId)
        {
 ...
 // ===== 查询接口 =====        public BuildingDef? GetBuildingDef(int id) => _buildingDefs.TryGetValue(id, out var d) ? d : null;
