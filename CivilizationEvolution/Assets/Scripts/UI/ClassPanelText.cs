...
{
 /// 阶层 UI 面板文本构建器（纯静态可测，TMP 富文本） /// 展示：整体社会概览 + 各阶层详情（人口/满足/忠诚/组织/影响/动荡/支持 + 8维需求条形图 + 主要不满）    public static class ClassPanelText
    {
 // —— 颜色常量（TMP 富文本）——        const string C_HEADER = "<color=#E8D5B7>";      // 标题色（暖金）
 ...
 /// <summary>需求维度中文名</summary>        public static string GetDimensionName(ClassNeedDimension d) => d switch
        {
 ...
 /// <summary>分数→颜色标签（0-45红 / 45-65橙 / 65+绿）</summary>        static string ScoreColor(float score)
        {
 ...
 /// <summary>生成文本条形图（含颜色）</summary>        static string Bar(float score)
        {
 ...
 /// <summary>整体社会概览（动荡分/主导阶层/最动荡阶层/总人口）</summary>        public static string BuildOverview(RealmSociety society)
        {
 ...
 /// <summary>单个阶层详情（核心指标 + 8维需求条形图 + 主要不满）</summary>        public static string BuildClassDetail(ClassProfile p)
        {
            if (p == null) return "";
            var sb = new StringBuilder();
            string clsName = ClassNames.Get(p.socialClass);

 // —— 标题行：阶层名 + 人口 + 占比 ——            sb.AppendLine($"{C_HEADER}▶ {clsName}{C_END}" +
                          $"  人口 {C_VALUE}{p.population:N0}{C_END}" +
                          $"（{C_VALUE}{p.populationShare * 100f:F1}%{C_END}）");

 // —— 核心指标行 ——            sb.AppendLine($"  满足 {ScoreColor(p.satisfaction)}{p.satisfaction:F0}{C_END}" +
 ...
 // —— 主要不满 ——            if (!string.IsNullOrEmpty(p.chiefGrievanceReason) || p.chiefGrievance != default)
            {
 ...
 // —— 8维需求条形图（2列布局，4行）——            if (p.needReport != null && p.needReport.dimensions != null && p.needReport.dimensions.Count > 0)
            {
                sb.AppendLine($"  {C_DIM}── 需求维度 ──{C_END}");
                var dims = new List<ClassNeedDimension>(p.needReport.dimensions.Keys);
 // 按权重顺序排列（枚举顺序即权重表顺序）                dims.Sort((a, b) => ((int)a).CompareTo((int)b));
 ...
 /// <summary>单个维度的"名称+条形图+分数"字符串</summary>        static string DimBar(ClassNeedDimension d, ClassNeedReport report)
        {
            float score = report.dimensions.TryGetValue(d, out var v) ? v.score : 0f;
 // 权重为0的维度（该阶层不关心）暗淡显示            bool relevant = score > 0f || report.dimensions.ContainsKey(d);
 ...
 /// <summary>构建完整阶层面板（概览 + 全部阶层详情，按人口占比降序）</summary>        public static string BuildFull(RealmSociety society)
        {
            var sb = new StringBuilder();
 ...
 // 按人口占比降序排列                var sorted = new List<KeyValuePair<GameEnums.SocialClass, ClassProfile>>(society.classes);
