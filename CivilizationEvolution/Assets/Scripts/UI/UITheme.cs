...
{
 /// UI 主题：统一色板 + 程序化圆角 Sprite + 按钮 ColorTint /// 供场景构建（编辑器工具）与运行时 UI（Toast 等）共用，保证视觉一致。 /// 纯代码生成，无外部图片资源。    public static class UITheme
    {
 // ===== 色板 =====        public static readonly Color PanelBg = new Color(0.07f, 0.09f, 0.15f, 0.88f);      // 面板底色（半透明）
 ...
 // ===== 事件日志分类色 =====        public static readonly Color LogSystem = new Color(0.72f, 0.78f, 0.88f, 1f);   // 系统
 ...
 // ===== 程序化圆角 Sprite ===== // 圆角九宫格尺寸（2026-09 升级：24/8→48/18——8px 圆角观感近方角—— // CK3/文明级面板圆角 16-20px——sliced 拉伸不影响圆角观感）        private const int RoundedSize = 48;
 ...
 /// <summary>面板/容器用圆角九宫格 Sprite（白底，配合 Image.color 着色）</summary>        public static Sprite RoundedPanelSprite
        {
            get { if (_panelSprite == null) _panelSprite = GenerateRoundedSprite(); return _panelSprite; }
        }

 /// <summary>按钮/Dropdown 用圆角九宫格 Sprite</summary>        public static Sprite RoundedButtonSprite
        {
            get { if (_buttonSprite == null) _buttonSprite = GenerateRoundedSprite(); return _buttonSprite; }
        }

 /// <summary>生成白底圆角九宫格 Sprite（带 1px 抗锯齿）</summary>        private static Sprite GenerateRoundedSprite()
        {
            ...
            for (int y = 0; y < RoundedSize; y++)
            {
                for (int x = 0; x < RoundedSize; x++)
                {
                    float lx = x + 0.5f;
                    float ly = y + 0.5f;
 // 到内部方形区域的距离（四角之外的部分参与圆弧判定）                    float dx = lx < r ? r - lx : (lx > RoundedSize - r ? lx - (RoundedSize - r) : 0f);
 ...
 /// <summary>为 Selectable 应用统一的 ColorTint 过渡（悬停/按下/选中/禁用）</summary>        public static void ApplyButtonTint(UnityEngine.UI.Selectable selectable)
        {
