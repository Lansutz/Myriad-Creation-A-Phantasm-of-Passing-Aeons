...
{
 /// MapRenderer.Texture —— 纹理更新（地图纹理重绘/强制刷新/获取纹理）（partial class，与 MapRenderer.cs 共享字段）    public partial class MapRenderer
    {

 /// <summary>更新地图纹理</summary>                private void UpdateMapTexture()
        {
            if (world == null || world.tiles == null) return;
            if (world.tiles.Length != mapWidth * mapHeight) return;

 // 复用像素缓冲区，仅在尺寸变化时重新分配            int pixelCount = mapWidth * mapHeight;
 ...
 // ===== 虚控制范围渲染（政治地图模式下，半透明三角形内部）=====            if (displayMode == MapDisplayMode.Political && _layerConfig.showVirtualControl)
            {
                DrawVirtualControlOverlay();
            }

 // ===== 聚落辐射范围渲染（政治地图模式下，半透明圆形）=====            if (displayMode == MapDisplayMode.Political && _layerConfig.showInfluenceRadius)
            {
                DrawInfluenceRadiusOverlay();
            }

 // ===== 占领/争议条纹叠加（45°斜线，不修改底色，仅叠加）=====            for (int i = 0; i < world.tiles.Length; i++)
            {
                if (!world.tiles[i].exists) continue;
                if (!IsOccupiedDisputed(world.tiles[i])) continue;
                int sx = i % mapWidth;
                int sy = i / mapWidth;
 // 45°斜线：(x + y) % spacing < spacing/2 时叠加条纹色                int spacing = Mathf.Max(2, _occupationStripeSpacing);
 ...
 // ===== 聚落独立图标渲染（可选叠加层，按等级大小+形态形状）=====            if (_layerConfig.showBurgMarkers && world.burgs != null && world.burgs.Count > 0)
            {
 ...
 // ===== 省份边界线（可选叠加层）=====            if (_layerConfig.showProvinceBorders && world.provinces != null && world.provinces.Count > 0)
            {
 ...
 // ===== 网格线（可选叠加层）=====            if (_layerConfig.showGrid)
            {
 ...
 // 编辑器画笔预览（编辑模式下高亮鼠标悬停的画笔范围）            if (_mapEditor != null && _mapEditor.IsEditMode && _hoverTile >= 0 && _mapEditor.CurrentTool != EditorTool.None)
            {
 ...
 /// <summary>虚控制范围计算结果缓存（避免每帧重算）</summary>        private VirtualControlResult _cachedVirtualControl;
        private int _cachedVirtualControlFrame = -1;

 /// <summary>绘制聚落独立图标（按等级大小+形态形状）</summary>        private void DrawSettlementIcon(BurgData burg, int pixelCount)
        {
            int bx = burg.tileIndex % mapWidth;
            int by = burg.tileIndex / mapWidth;

 // 图标大小根据等级：Ⅰ=2px, Ⅱ=3px, Ⅲ=4px, Ⅳ=5px, Ⅴ=6px            int level = (int)burg.settlementLevel;
            int iconSize = Mathf.Clamp(level + 1, 2, 6);
            if (mapWidth > 512) iconSize = Mathf.Clamp(iconSize + 1, 3, 7);

 // 图标颜色根据类型            Color iconColor = burg.type switch
            {
 ...
 // 根据聚落形态决定图标形状                    bool draw = burg.settlementType switch
                    {
 // 城：方形图标                        SettlementType.City => true,
 // 堡：菱形/十字图标                        SettlementType.Fort => Mathf.Abs(dx) + Mathf.Abs(dy) <= half + 1,
 // 村镇：圆形图标                        _ => dx * dx + dy * dy <= half * half + 1
                    };

                    if (draw)
                    {
                        int pi = py * mapWidth + px;
 // 边缘用深色描边                        bool isEdge = (dx == -half || dx == half || dy == -half || dy == half);
 ...
 /// <summary>绘制虚控制范围叠加（半透明三角形内部）</summary>        private void DrawVirtualControlOverlay()
        {
 // 缓存计算结果（同一帧内不重复计算）            int frame = Time.frameCount;
 ...
 // 虚控制地块叠加半透明颜色（按政权颜色）            foreach (var kv in _cachedVirtualControl.virtualControlTiles)
            {
                int tileIdx = kv.Key;
                int realmId = kv.Value;
                if (tileIdx < 0 || tileIdx >= _pixelBuffer.Length) continue;
 // 已经是实际占领的地块不叠加（只叠加未占领的虚控制区）                if (world.tiles[tileIdx].ownerRealmId == realmId) continue;

                Color realmColor = GetRealmColor(realmId);
 // 虚控制用更低的透明度（0.15），区分于实际占领                _pixelBuffer[tileIdx] = Color.Lerp(_pixelBuffer[tileIdx], realmColor, 0.15f);
            }

 // 绘制控制点之间的连接线（虚控制线）            foreach (var (a, b) in _cachedVirtualControl.connections)
            {
 ...
 /// <summary>绘制聚落辐射范围叠加（半透明圆形）</summary>        private void DrawInfluenceRadiusOverlay()
        {
 ...
 // 辐射半径根据等级：Ⅰ=1, Ⅱ=2, Ⅲ=3, Ⅳ=4, Ⅴ=5                int radius = (int)burg.settlementLevel;
                ...
                for (int dy = -radius; dy <= radius; dy++)
                {
                    for (int dx = -radius; dx <= radius; dx++)
                    {
                        ...
                        if (px < 0 || px >= mapWidth || py < 0 || py >= mapHeight) continue;
                        int pi = py * mapWidth + px;
 // 辐射范围用极低透明度（0.08），只做视觉暗示                        float distFactor = 1f - (float)(dx * dx + dy * dy) / (radius * radius);
 ...
 /// <summary>在纹理上画直线（Bresenham）</summary>        private void DrawLineOnTexture(int x0, int y0, int x1, int y1, Color color)
        {
 ...
 /// <summary>获取当前地图纹理（用于导出PNG）</summary>        public Texture2D GetMapTexture() => mapTexture;
