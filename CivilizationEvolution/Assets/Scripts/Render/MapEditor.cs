...
{
 /// 地图编辑器工具类型 /// 对齐 FantasyMapSimulator: CustomMapImageLayer / MapBrush / cursorPixelPosition / CeilToPixelGrid

 /// 地图编辑器 /// 运行时像素级地图编辑：地形绘制、省份边界调整、子地块放置 /// 支持画笔大小、撤销/重做、绘制后自动重算省份/海洋/Burg    public class MapEditor
    {
        private readonly GameWorld _world;
        private readonly MapRenderer _renderer;

 // ===== 编辑器状态 =====        public bool IsEditMode { get; private set; }
 ...
 // ===== 撤销/重做 =====        private class EditAction
        {
 ...
 // ===== 模式切换 =====        public void ToggleEditMode()
        {
 ...
 // ===== 绘制入口（由MapRenderer的鼠标事件调用）=====        public void OnPaintStart(int tileIndex)
        {
 ...
 // 绘制结束后重算相关系统            RecalculateAfterEdit();
        }

 // ===== 核心绘制逻辑 =====        private void PaintAt(int centerTile)
        {
            ...
            for (int dy = -r; dy <= r; dy++)
            {
                ...
                {
 // 圆形画笔（方形画笔的话去掉距离判断）                    if (dx * dx + dy * dy > r * r) continue;

                    int px = cx + dx;
                    int py = cy + dy;

 // 地图环绕：先环绕再边界检查（柱面=左右环绕，环面=全环绕，平面=不环绕）                    if (_world.wrapMode == MapWrapMode.Cylindrical || _world.wrapMode == MapWrapMode.Toroidal)
                        px = (px + _world.mapWidth) % _world.mapWidth;
                    if (_world.wrapMode == MapWrapMode.Toroidal)
                        py = (py + _world.mapHeight) % _world.mapHeight;

 // 平面模式下的边界检查（环绕模式下坐标已合法）                    if (_world.wrapMode == MapWrapMode.Flat)
                    {
 ...
 // 只在中心点放置Burg（画笔范围内的第一个陆地地块）                    if (tile.isLand && !HasBurgAt(tileIndex))
                    {
 ...
 // ===== Burg 放置/移除 =====        private bool HasBurgAt(int tileIndex)
        {
 ...
 // ===== 绘制后重算 =====        private void RecalculateAfterEdit()
        {
 // 1. 重算省份成员列表            if (_world.provinces != null)
            {
 ...
 // 2. 重算海洋连通（简化：标记海洋地块） // 完整的flood fill在SeaLandGenerator中，这里只做基础标记            for (int i = 0; i < _world.tiles.Length; i++)
            {
 ...
 // 3. 强制刷新地图渲染            if (_renderer != null)
                _renderer.ForceRefresh();

            Debug.Log("[MapEditor] 绘制后重算完成");
        }

 // ===== 撤销/重做 =====        public void Undo()
        {
        ...
        private void ApplyActionForward(EditAction action)
        {
            ...
            {
 // 重新添加（从burgChanges中找）                foreach (var (bid, burg, _) in action.burgChanges)
                    if (bid == id && burg != null)
        ...
        private void PushUndo(EditAction action)
        {
            ...
            {
 // 移除最旧的（Stack不支持直接移除底部，用临时栈）                var temp = new Stack<EditAction>();
 ...
 // ===== 工具描述 =====        private string GetToolDescription()
        {
 ...
 // ===== 状态查询 =====        public int UndoCount => _undoStack.Count;
        public int RedoCount => _redoStack.Count;
        public bool IsPainting => _isPainting;

 /// <summary>获取画笔覆盖的地块列表（用于渲染预览）</summary>        public List<int> GetBrushTileIndices(int centerTile)
        {
 ...
 // ===== 海洋地块重算 ===== /// 重新计算所有海洋地块的等级（海岸/近海/中海/远海/深海） /// 使用多源BFS从陆地出发计算每个海洋格到最近陆地的距离 /// 绘制完陆地后调用此方法，用于海军和贸易系统        public void RecalculateOceanZones()
        {
            int w = _world.mapWidth;
            int h = _world.mapHeight;
            int total = w * h;

 // 距离数组：-1表示未访问，陆地格距离为0            int[] dist = new int[total];
            for (int i = 0; i < total; i++) dist[i] = -1;

 // BFS队列：先把所有陆地格入队（距离0）            var queue = new Queue<int>();
 ...
 // 4方向邻居（上下左右）            int[] dx = { 0, 0, -1, 1 };
            int[] dy = { -1, 1, 0, 0 };

 // 多源BFS            while (queue.Count > 0)
            {
 ...
 // 左右环绕（柱面/环面）                    if (_world.wrapMode == MapWrapMode.Cylindrical || _world.wrapMode == MapWrapMode.Toroidal)
                        nx = (nx + w) % w;
                    if (_world.wrapMode == MapWrapMode.Toroidal)
                        ny = (ny + h) % h;

 // 平面模式边界检查                    if (_world.wrapMode == MapWrapMode.Flat)
                    {
 ...
 // 根据距离设置海洋等级            int coastCount = 0, nearCount = 0, midCount = 0, farCount = 0, deepCount = 0;
            for (int i = 0; i < total; i++)
            {
 ...
 /// <summary>获取指定海洋等级的地块数量</summary>        public int GetOceanTileCount(GameEnums.OceanTier tier)
        {
            int count = 0;
            for (int i = 0; i < _world.tiles.Length; i++)
            {
