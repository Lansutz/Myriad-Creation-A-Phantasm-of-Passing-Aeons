...
{
 /// 革新分类体系 /// 两级分类：大类（性质维度）× 子类（领域维度） /// 大类四分类 = 文化学经典四分法：物质文化（技术）/ 精神文化（思维）/ /// 制度文化（制度）/ 习俗文化（传统） /// 边界归位规则：武器装备器械→技术；战术兵法→思维；军制→制度； /// 医术实践→技术-医疗；医理→思维；教义→思维-神学；仪式→传统-仪礼；教团→制度-教制
 /// <summary>革新大类（性质维度）</summary>    public enum InnovationDomain
    {
 ...
 /// <summary>革新子类（领域维度；每个子类唯一归属一个大类）</summary>    public enum InnovationField
    {
 // ===== 技术 Technology =====        Mining,             // 采掘
        Agriculture,        // 农耕
        Metallurgy,         // 冶炼
        Construction,       // 营建
        Medicine,           // 医疗
        ...
        Craft,              // 工艺
        Transport,          // 交通
 // ===== 思维 Thought =====        Script,             // 文字
        Mathematics,        // 数理
        Philosophy,         // 哲思
        NaturalCognition,   // 格致（自然认知）
        Theology,           // 神学
        MilitaryThought,    // 兵学（战术兵法）
 // ===== 制度 Institution =====        Governance,         // 政制
        Law,                // 法制
        Bureaucracy,        // 官制
        Economic,           // 经制
 ...
 /// <summary>子类→大类归属映射（子类唯一归属，推导 domain 用）</summary>    public static class InnovationDomainMap
    {
        public static readonly Dictionary<InnovationField, InnovationDomain> FieldToDomain =
            ...
            {
 // 技术                [InnovationField.Mining] = InnovationDomain.Technology,
 ...
 // 思维                [InnovationField.Script] = InnovationDomain.Thought,
 ...
 // 制度                [InnovationField.Governance] = InnovationDomain.Institution,
 ...
 // 传统                [InnovationField.Ritual] = InnovationDomain.Tradition,
 ...
 /// <summary>子类推导大类（未注册返回 Technology 兜底，测试可校验）</summary>        public static InnovationDomain GetDomain(InnovationField field)
        {
            return FieldToDomain.TryGetValue(field, out var domain) ? domain : InnovationDomain.Technology;
        }
    }

 /// <summary>革新定义（数据驱动：Innovation/Innovations.json，Base/Mods 可覆盖）</summary>    [Serializable]
    public class InnovationDef
    {
        public int innovationId;
 /// <summary>内置名称（本地化表有 &lt;id&gt;_name 键时优先）</summary>        public string innovationName;
 /// <summary>子类（domain 由映射表推导）</summary>        public InnovationField field;
        public int era; // 时代 0-5
        public float researchCost;
 /// <summary>前置（AND：全部满足）</summary>        public List<int> prerequisites = new List<int>();
 /// <summary>或前置（OR：满足任一即可；差异化路径——同节点多形态）</summary>        public List<int> prerequisitesAny = new List<int>();
 ...
 /// <summary>写实描述：本地化表优先（&lt;id&gt;_desc），回退内嵌字段</summary>        public string GetDescription() => Localization.Has(innovationId + "_desc") ? Localization.Get(innovationId + "_desc") : description;
    }
}
