...
{
 /// 球形行星地形生成器（专业版） /// 算法集成： /// 1. 3D球面Simplex噪声 + 域扭曲Domain Warping + 山脊噪声RidgedFbm /// 2. 分位数归一化（Quantile Normalization）——确保海陆比例精准 /// 3. 球面Voronoi图（Spherical Voronoi）——板块构造边界 /// 4. Priority-Flood水文模拟——洼地填充+排水方向+汇水面积+河流等级 /// 5. Holdridge生命地带分类——生物温度+降水+潜在蒸散比→55群系 /// 6. 多线程Parallel.For优化——百万级地块秒级生成 /// 平面/球形可切换：数据层统一用球面坐标生成，渲染层可选平面等矩形或3D球体    public class PlanetTerrainGenerator
    {
        private readonly SphericalNoise _noise;
        private readonly int _seed;

 // ===== 生成参数（可调节，借鉴Gleba量级）=====        public float TargetLandFraction = 0.30f;   // 目标陆地比例（地球≈29%）
 ...
 /// 生成完整地形（填充TileData数组） /// 专业版流程：原始高程→分位数归一化→板块边界→海陆→物理属性→水文→Holdridge群系        public void Generate(TileData[] tiles, int width, int height)
        {
 ...
 // ===== 第0步：预计算球面坐标（所有点的单位球面3D坐标）=====            var sphereX = new float[n];
 ...
 // ===== 第1步：生成原始高程（球面噪声+域扭曲+山脊）=====            var rawElevation = new float[n];
            ParallelLoop(0, n, i =>
            {
 ...
 // 山脊噪声叠加（制造山脉走向）                float ridge = _noise.RidgedFbm(
 ...
 // ===== 第2步：分位数归一化——确保海陆比例精准 =====            float seaLevel = QuantileNormalize(rawElevation, TargetLandFraction);
            Debug.Log($"[PlanetTerrainGenerator] 分位数归一化完成，海平面阈值={seaLevel:F4}，目标陆地比例={TargetLandFraction}");

 // ===== 第3步：球面Voronoi板块构造 =====            var voronoi = new SphericalVoronoi(_seed + 1000);
 ...
 // 板块边界叠加山脉（汇聚边界造山，简化：所有边界都有一定抬升）            ParallelLoop(0, n, i =>
            {
 ...
 // ===== 第5步：坡度、海岸、温度、降水 =====            ParallelLoop(0, n, i =>
            {
                int x = i % width;
                int y = i / width;
                ref TileData tile = ref tiles[i];

 // 坡度（4邻域最大高程差）                float maxDiff = 0f;
 ...
 // 温度（纬度主导+高程递减+海陆调节）                float lat = 90f - (y / (float)height) * 180f;
 ...
 // 降水（三圈环流+地形抬升+大陆性）                float absLat = Mathf.Abs(lat);
 ...
 // 准备isLand数组（后续多个算法使用）            var isLandArray = new bool[n];
            for (int i = 0; i < n; i++) isLandArray[i] = tiles[i].isLand;

 // ===== 第5.5步：水力侵蚀（修改高程，形成河谷/峡谷/冲积扇）=====            var erosion = new HydraulicErosion(width, height, _seed + 2000);
 ...
 // 侵蚀后重新计算坡度和温度（高程已改变）            ParallelLoop(0, n, i =>
            {
                int x = i % width;
 ...
 // 重新计算温度（高程已改变）                float lat = 90f - (y / (float)height) * 180f;
                float latFactor = Mathf.Cos(lat * Mathf.Deg2Rad);
                float baseTemp = -10f + latFactor * 38f;
                float elevationCooling = tile.elevation01 * 65f;
 ...
 // ===== 第5.8步：大气环流GCM（气压带+三圈环流+科里奥利+季风+地形降水）=====            var tempArray = new float[n];
 ...
 // ===== 第5.9步：洋流模拟（风生环流+科里奥利+大陆阻挡+暖流寒流）=====            var ocean = new OceanCurrentSimulator(width, height);
 ...
 // ===== 第6步：Priority-Flood水文模拟 =====            var hydro = new HydrologySystem(width, height, wrapX: true);
 ...
 // ===== 第6.5步：PCA地形特征提取（地形分类+大陆主轴）=====            var pca = new TerrainPCA(width, height);
 ...
 // ===== 第7步：Holdridge生命地带群系分类 + 肥力 =====            ParallelLoop(0, n, i =>
            {
                ref TileData tile = ref tiles[i];
 ...
 // ===== 分位数归一化 ===== /// 分位数归一化：将原始高程映射到0-1，使超过阈值的陆地比例等于targetLandFraction /// 原理：排序找到targetLandFraction分位数作为海平面，然后线性映射        private float QuantileNormalize(float[] elevation, float targetLandFraction)
        {
 ...
 // 找到目标分位数对应的高程值作为海平面            int seaLevelIndex = Mathf.Clamp((int)(n * (1f - targetLandFraction)), 0, n - 1);
            float seaLevel = sorted[seaLevelIndex];

 // 线性映射：海平面→0.5，最小值→0，最大值→1            float min = sorted[0];
            ...
            for (int i = 0; i < n; i++)
            {
 // 以海平面为中心，向两侧拉伸                float normalized;
 ...
 // ===== 气候带分类 =====        private static GameEnums.ClimateZone ClassifyClimateZone(float temp, float precip)
        {
 ...
 // ===== 工具 =====        private void ParallelLoop(int from, int to, Action<int> body)
        {
