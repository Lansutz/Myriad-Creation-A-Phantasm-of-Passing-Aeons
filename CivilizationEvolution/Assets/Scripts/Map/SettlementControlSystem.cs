...
namespace CivilizationEvolution.Map
{
    ...
    /// 1. 每个聚落有等级(Ⅰ-Ⅴ)和辐射范围
    /// 2. 同等级聚落不互相控制（平等关系）
    /// 3. 高等级聚落对辐射范围内低等级聚落有控制/影响力
    /// 4. 控制进度随时间增加，速度取决于驻扎部队的质量和数量
    /// 5. 打下高等级聚落后，随时间推移其下属聚落控制逐步流失
    /// 6. 不驻扎部队 → 控制进度倒退，最终叛乱/独立
    ...
    public static class SettlementControlSystem
    {
        ...
        public const float BaseControlSpeed = 0.5f;
        /// <summary>控制进度完成阈值（到100即完成控制）</summary>
        public const float ControlCompleteThreshold = 100f;
        /// <summary>无驻扎时控制进度每日倒退速度</summary>
        public const float NoGarrisonDecaySpeed = 1f;
        /// <summary>宗主被占领后，下属聚落控制流失加速系数</summary>
        public const float SuzerainOccupiedDecayMultiplier = 3f;
        /// <summary>控制稳固后流失速度减半</summary>
        public const float StableDecayMultiplier = 0.5f;
            ...
            Dictionary<int, CivilizationEvolution.War.Army> armies = null)
        {
            ...
            // 1. 更新每个聚落的驻扎部队信息
            UpdateGarrisonInfo(burgs, tiles, armies);

            // 2. 扫描控制关系（高等级 → 低等级）
            ScanControlRelationships(burgs, tiles, mapWidth, mapHeight);

            // 3. 更新控制进度
            UpdateControlProgress(burgs, tiles);

            // 4. 检查控制完成/流失
            CheckControlCompletion(burgs);
        }
        ...
        private static void UpdateControlProgress(Dictionary<int, BurgData> burgs, TileData[] tiles)
        {
            foreach (var controller in burgs.Values)
            {
                ...
                foreach (int targetId in controller.controlledBurgIds)
                {
                    ...
                    // 计算控制速度
                    float speed = CalculateControlSpeed(controller, target, tiles);

                    if (suzerainOccupied)
                    {
                        // 宗主被占领：控制加速流失
                        float decay = NoGarrisonDecaySpeed * SuzerainOccupiedDecayMultiplier;
                    ...
                    {
                        // 无驻扎：控制进度倒退
                        float decay = NoGarrisonDecaySpeed;
                        if (target.isControlStable) decay *= StableDecayMultiplier;
                        progress = Mathf.Max(0f, progress - decay);
                    }
                    else
                    {
                        // 有驻扎：控制进度增长
                        progress = Mathf.Min(ControlCompleteThreshold, progress + speed);
                    }
        ...
        private static float CalculateControlSpeed(BurgData controller, BurgData target, TileData[] tiles)
        {
            ...
            // 驻扎部队数量修正：越多越快（上限200%）
            float quantityFactor = Mathf.Clamp(1f + controller.garrisonQuantity / 500f, 1f, 3f);
            speed *= quantityFactor;

            // 驻扎部队质量修正：训练度越高越快（上限150%）
            float qualityFactor = 0.7f + controller.garrisonQuality / 100f * 0.8f;
            speed *= qualityFactor;

            // 等级差修正：等级差越大控制越快
            int levelDiff = controller.settlementLevel - target.settlementLevel;
