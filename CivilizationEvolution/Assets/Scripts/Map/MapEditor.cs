...
{
 /// 地图编辑器（重做版） /// 支持自由画笔绘制、增删地块、地形编辑、左右连通 /// 不再必须沿格子作画，画笔可为圆形/方形/自定义半径    public class MapEditor : MonoBehaviour
    {
        [Header("引用")]
 ...
 /// <summary>编辑器是否激活（只读）</summary>        public bool IsEditorActive => isEditorActive;
 /// <summary>当前画笔信息</summary>        public BrushMode CurrentBrushMode => brushMode;

 // 拖拽状态        private bool _isPainting = false;
 ...
 /// <summary>更新鼠标悬停地块</summary>        private void UpdateHoveredTile()
        {
            if (mapRenderer == null) return;
            hoveredTile = mapRenderer.ScreenToTile(Input.mousePosition);
        }

 /// <summary>更新画笔预览（当前画笔影响的地块）</summary>        private void UpdateBrushPreview()
        {
 ...
 /// <summary>处理绘制输入</summary>        private void HandlePaintInput()
        {
 // 左键按下开始绘制            if (Input.GetMouseButtonDown(0))
            {
                _isPainting = true;
 ...
 // 左键松开停止绘制            if (Input.GetMouseButtonUp(0))
            {
                _isPainting = false;
            }

 // 拖拽连续绘制            if (_isPainting && continuousPaint && Input.GetMouseButton(0))
            {
 ...
 /// <summary>处理键盘快捷键</summary>        private void HandleKeyboardShortcuts()
        {
 // 括号键调整画笔大小            if (Input.GetKeyDown(KeyCode.LeftBracket))
                brushRadius = Mathf.Max(0, brushRadius - 1);
            if (Input.GetKeyDown(KeyCode.RightBracket))
                brushRadius = Mathf.Min(20, brushRadius + 1);

 // 数字键切换画笔模式            if (Input.GetKeyDown(KeyCode.Alpha1)) brushMode = BrushMode.RaiseTerrain;
 ...
 // B键切换画笔形状            if (Input.GetKeyDown(KeyCode.B))
                brushShape = (brushShape == BrushShape.Circle) ? BrushShape.Square : BrushShape.Circle;

 // E键切换编辑器激活            if (Input.GetKeyDown(KeyCode.E))
                ToggleEditor();
        }

 /// <summary>应用画笔到当前悬停位置</summary>        public void ApplyBrush()
        {
 ...
 // 标记脏区，触发增量重算            world.RecalculateDirty();
        }

 /// <summary>抬高地形</summary>        private void RaiseTerrain(int tileIndex)
        {
 ...
 /// <summary>降低地形</summary>        private void LowerTerrain(int tileIndex)
        {
            if (!world.tiles[tileIndex].exists) return;
            ref TileData tile = ref world.tiles[tileIndex];
            tile.elevation01 = Mathf.Clamp(tile.elevation01 - brushStrength, -1f, 1f);
 ...
 /// <summary>设置为陆地（直接修改isLand，跳过海平面判定）</summary>        private void SetLand(int tileIndex)
        {
            if (!world.tiles[tileIndex].exists) return;
            ref TileData tile = ref world.tiles[tileIndex];
            tile.isLand = true;
 ...
 /// <summary>设置为海洋</summary>        private void SetOcean(int tileIndex)
        {
            if (!world.tiles[tileIndex].exists) return;
            ref TileData tile = ref world.tiles[tileIndex];
            tile.isLand = false;
 ...
 /// <summary>平滑地形（与邻接取平均）</summary>        private void SmoothTerrain(int tileIndex)
        {
            if (!world.tiles[tileIndex].exists) return;
 ...
 // ===== 公共接口 =====
 /// <summary>切换编辑器激活状态</summary>        public void ToggleEditor()
        {
            isEditorActive = !isEditorActive;
            Debug.Log($"[MapEditor] 编辑器{(isEditorActive ? "激活" : "关闭")}");
        }

 /// <summary>设置画笔模式</summary>        public void SetBrushMode(BrushMode mode)
        {
            brushMode = mode;
            Debug.Log($"[MapEditor] 画笔模式: {mode}");
        }

 /// <summary>设置画笔形状</summary>        public void SetBrushShape(BrushShape shape)
        {
            brushShape = shape;
        }

 /// <summary>设置画笔半径</summary>        public void SetBrushRadius(int radius)
        {
            brushRadius = Mathf.Clamp(radius, 0, 20);
        }

 /// <summary>切换左右连通</summary>        public void ToggleWrapX()
        {
 ...
 /// <summary>填充整个地图为存在地块</summary>        public void FillAllTiles()
        {
 ...
 /// <summary>清除所有地块（全部设为虚空）</summary>        public void ClearAllTiles()
        {
            for (int i = 0; i < world.tiles.Length; i++)
            {
                world.RemoveTile(i);
 ...
 /// <summary>获取当前画笔信息（用于UI显示）</summary>        public string GetBrushInfo()
        {
