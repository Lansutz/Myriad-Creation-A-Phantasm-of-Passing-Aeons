...
namespace CivilizationEvolution.Map
{
 ...
 /// 聚落等级（Ⅰ-Ⅴ级） /// 对应：村落→集镇→城邑→都会→大都会    public enum SettlementLevel
    {
        LevelI = 0,    // Ⅰ级 聚落（村落）
        LevelII = 1,   // Ⅱ级 集镇
        LevelIII = 2,  // Ⅲ级 城邑
 ...
 /// 聚落功能类型（文明引擎IN5城镇功能类型学） /// 一个聚落可同时拥有多种功能，主功能决定发展倾向    [Flags]
 ...
 /// 城市重心（都会级别Ⅳ-Ⅴ级的核心发展方向） /// 决定城市建筑布局、经济结构、军事价值    public enum CityFocus
    {
 ...
 /// 城的形态（平面布局形态） /// 由地形、建城时代、文化传统共同决定    public enum CityForm
    {
 ...
 /// 堡垒亚型（堡的具体形态，文档4种区域堡寨+关口堡）    public enum FortSubtype
    {
 // ===== 关口堡（Pass）：通道控制 =====        PassFort,           // 关口堡：峡谷/隘口/狭河谷通道控制

 // ===== 区域堡寨（Regional）4种亚型 =====        HighlandKeep,       // 高地堡：丘陵高地，区域压制网，堡垒链
        ManorFort,          // 坞堡庄园：内地农耕区，自给自足，基层治理节点
        PlainGarrison,      // 平原屯堡：开阔平原边境，沿边境线防御带
        EstuaryFort,        // 河口堡：大河河口/干流要道，管控内河与近海航线

 // ===== 其他堡垒形态 =====        StarFort,           // 棱堡：火炮时代，星形多角度防御
 ...
 /// 港口层级（文明引擎IN1）    public enum PortTier
    {
        None = 0,
 ...
 /// 关隘瓶颈类型（文明引擎IN2）    public enum BottleneckType
    {
        None = 0,
 ...
 /// 聚落升级路线（决定聚落从低级向高级演化的路径依赖） /// 不同起点的聚落有不同的升级路径和最终形态    public enum UpgradePath
    {
 ...
 /// 聚落演化阶段（用于渐进式演化，不突变）    public enum EvolutionStage
    {
 ...
 /// 城墙等级（防御工事强度）    public enum WallLevel
    {
        None = 0,           // 无城墙（村落/集镇常见）
