...
{
 /// 继承法系统（四轴独立选择） /// 轴1 继承范围：限本族 vs 血亲不论姓氏 /// 轴2 支系：长支优先（长房） vs 兄终弟及（横向） /// 轴3 性别：男子优先/男子专属/男女平等/女子优先/女子专属 /// 轴4 长幼：年长者先 vs 年幼者先 /// 组合成具体继承法：如长子继承 = 血亲+长支+男子优先+年长； /// 游牧汗国 = 血亲+兄终弟及+男子专属+年长；幼子守灶 = 年幼先
 /// <summary>轴1·继承范围</summary>    public enum InheritanceScope
    {
        ClanOnly,       // 限于本族：继承权仅限宗族/家族内（familyId 相同）
        CognaticKin     // 血亲不论姓氏：同血缘即可（父系/母系血亲链）
    }

 /// <summary>轴2·支系</summary>    public enum InheritanceBranch
    {
        EldestLine,     // 长支优先：长房/嫡系优先于旁支
        Collateral      // 兄终弟及：横向——同辈兄弟优先，然后下一代（游牧汗国式）
    }

 /// <summary>轴3·性别</summary>    public enum InheritanceGender
    {
 ...
 /// <summary>轴4·长幼</summary>    public enum InheritanceAge
    {
        Seniority,      // 年长者先
        Juniority       // 年幼者先（幼子守灶）
    }

 /// 头衔继承模式（分配维度——称号怎么分；Elective 移除—— /// 选举是产生方式（A1 交接方式表达），头衔模式只表达分配） /// SoleHeir=唯一继承（称号整体传予一人）/ FamilyShared=家族共享（法兰克： /// 所有兄弟共享"法兰克之王"王号，各自统治领地部分）    public enum TitleInheritanceMode
    {
        SoleHeir,       // 唯一继承：头衔整体传予一人
        FamilyShared    // 家族共享：头衔由全体继承人共同领有（墨洛温/加洛林：诸子皆王）
    }

 /// 领地继承模式（学术：土地如何分配） /// Primogeniture=长子独得/ Partible=诸子均分（西班牙1157-1300、中国析产）/ /// Ultimogeniture=幼子守灶/ Seniority=兄终弟及/ /// Entail=限定继承不可分割（英国 strict settlement、日本家督）/ /// Collective=家族共有（印度 mitakshara joint family）    public enum LandInheritanceMode
    {
 ...
 /// <summary>继承法定义（四轴人序 + 头衔模式 + 领地模式，相互解耦）</summary>    [Serializable]
 ...
 /// <summary>头衔继承模式（头衔如何传承）</summary>        public TitleInheritanceMode titleMode = TitleInheritanceMode.SoleHeir;
 /// <summary>领地继承模式（土地如何分配）</summary>        public LandInheritanceMode landMode = LandInheritanceMode.Primogeniture;
 ...
 // ===== 经典组合 =====
 /// <summary>长子继承（西欧晚期封建）：血亲+长支+男子优先+年长；头衔唯一+领地长子独得</summary>        public static InheritanceLaw Primogeniture() =>
 ...
 /// <summary>幼子守灶（部分游牧/山区传统）：年幼先；头衔唯一+领地幼子独得</summary>        public static InheritanceLaw Ultimogeniture() =>
 ...
 /// <summary>兄终弟及（游牧汗国/早期王室）：血亲+横向+男子专属+年长；领地按人序轮转</summary>        public static InheritanceLaw Tanistry() =>
 ...
 /// <summary>母系继承（女子专属）：血亲+长支+女子专属+年长</summary>        public static InheritanceLaw Matrilineal() =>
            new InheritanceLaw(InheritanceScope.CognaticKin, InheritanceBranch.EldestLine,
                InheritanceGender.FemaleOnly, InheritanceAge.Seniority,
                TitleInheritanceMode.SoleHeir, LandInheritanceMode.Primogeniture);

 /// 法兰克式（墨洛温/加洛林传统）：王号家族共享（诸子皆"法兰克之王"） /// + 领地均分（凡尔登 843 三分的是领地，头衔仍为家族共有）        public static InheritanceLaw FrankishPartible() =>
            new InheritanceLaw(InheritanceScope.CognaticKin, InheritanceBranch.EldestLine,
                InheritanceGender.MaleOnly, InheritanceAge.Seniority,
                TitleInheritanceMode.FamilyShared, LandInheritanceMode.Partible);

 /// <summary>萨利克法（法兰克）：男子专属+均分继承（排斥女性≠长子继承）</summary>        public static InheritanceLaw Salic() =>
            new InheritanceLaw(InheritanceScope.CognaticKin, InheritanceBranch.EldestLine,
                InheritanceGender.MaleOnly, InheritanceAge.Seniority,
                TitleInheritanceMode.SoleHeir, LandInheritanceMode.Partible);

 /// <summary>中国式（唐代定论：宗祧与析产分离）：头衔嫡长子唯一（宗祧/官爵），领地诸子均分（析产）</summary>        public static InheritanceLaw ChinesePartible() =>
            new InheritanceLaw(InheritanceScope.ClanOnly, InheritanceBranch.EldestLine,
                InheritanceGender.MalePreference, InheritanceAge.Seniority,
                TitleInheritanceMode.SoleHeir, LandInheritanceMode.Partible);

 /// <summary>选举王（波兰自由选王/教宗）——注意：选举是产生方式（A1 交接方式表达）， /// 继承法（世袭内部规则）不含选举；此方法保留作兼容参考，头衔模式已无 Elective</summary>        [System.Obsolete("选举是产生方式（A1 交接方式），继承法不含选举——使用 GovernmentComposition 的 supremeSuccession 表达")]
        public static InheritanceLaw ElectiveMonarchy() =>
            new InheritanceLaw(InheritanceScope.CognaticKin, InheritanceBranch.EldestLine,
                InheritanceGender.Equal, InheritanceAge.Seniority,
                TitleInheritanceMode.SoleHeir, LandInheritanceMode.Primogeniture);

 /// <summary>继承法名称（中文）</summary>        public string GetName()
        {
 ...
 /// 领地分配（学术：LandInheritanceMode 的分配逻辑） /// 给定继承人数与领地数，返回分配方案（每人应得领地数；长子独得/幼子独得/限定=首位全得）        public int[] DistributeLand(int heirCount, int landCount)
        {
 ...
 // 诸子均分：按人数均分，余数归首位                    int baseShare = landCount / heirCount;
 ...
 // 幼子守灶：全部归末位（人序末位=幼子）                    result[heirCount - 1] = landCount;
                    break;
                case LandInheritanceMode.Collective:
 // 家族共有：名义上归集体（首位代管登记）                    result[0] = landCount;
                    break;
                default:
 // Primogeniture/Seniority/Entail：首位（继承人）全得                    result[0] = landCount;
                    break;
            }
            return result;
        }

 /// 依继承法从候选人中确定继承人（纯判定：过滤+排序后返回首位，无合格者返回 null） /// 判定顺序：范围过滤 → 性别过滤/偏好 → 支系 → 长幼        public CharacterData DetermineHeir(List<CharacterData> candidates, CharacterData currentRuler)
        {
 ...
 /// 继承候选过滤+排序池（轴1 范围/轴3 性别硬过滤 + 复合排序： /// 性别偏好 > 支系 > 长幼）——DetermineHeir 与继承线判定共用        public List<CharacterData> GetHeirOrderedPool(List<CharacterData> candidates, CharacterData currentRuler)
        {
            if (candidates == null || candidates.Count == 0) return null;

            var pool = new List<CharacterData>(candidates);

 // 轴1 范围过滤：限本族——仅保留与统治者同家族者            if (scope == InheritanceScope.ClanOnly && currentRuler != null)
            {
                pool.RemoveAll(c => c.familyId != currentRuler.familyId);
            }

 // 轴3 性别过滤（专属型硬过滤）            if (gender == InheritanceGender.MaleOnly)
 ...
 // 复合排序（单一比较器，优先级：性别偏好 > 支系 > 长幼）            pool.Sort((a, b) =>
            {
 // 轴3 性别偏好（优先型软排序，最高优先）                if (gender == InheritanceGender.MalePreference)
                {
 ...
 // 轴2 支系：长支（同族优先于旁支）vs 兄终弟及（同辈=与统治者共享父母者优先）                if (currentRuler != null)
                {
 ...
 // 轴4 长幼排序                return age == InheritanceAge.Seniority ? b.age.CompareTo(a.age) : a.age.CompareTo(b.age);
            });

            return pool;
        }

 /// <summary>是否与指定角色为同辈兄弟（共享任一父母；无父母记录不算兄弟）</summary>        private static bool IsSibling(CharacterData a, CharacterData ruler)
        {
