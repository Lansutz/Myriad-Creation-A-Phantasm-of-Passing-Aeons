...
{
 /// 政体效果门面：负责把当前政体成分（GovernmentComposition）换算成数值效果并缓存， /// 以及在政权创建 / 手动改制时应用这些效果。 /// 重要设计边界（历史制度主义）： /// 本系统【不做】"压力累积到阈值就自动切换政体"的机械演化。 /// 政体成分的实际变更只有两条合法路径： /// 1) RegimeChangeDynamics：结构张力只定义"可能性"，关键节点窗口内派系博弈后 /// 经 GovernmentReform.Reform（含革新可行性硬约束）落地； /// 2) 玩家手动改制（ChangeGovernment，UI 层已先校验革新前置）。 /// 本类每 Tick 只做一件事：让缓存效果与可能被上述路径改变的成分保持同步。    public class GovernmentSystem
    {
 ...
 // 上一次同步时的政体成分指纹，用于识别"成分是否被外部路径改变"        private readonly Dictionary<int, string> _lastFingerprint = new Dictionary<int, string>();
 ...
 /// <summary>政权创建时初始化政体效果</summary>        public void InitializeRealmGovernment(RealmData realm)
        {
 ...
 /// <summary>把政体效果应用到政权数据（仅在初始化 / 成分确实变化 / 手动改制时调用，保证幂等）</summary>        private void ApplyGovernmentEffects(RealmData realm, GovernmentEffects.GovernmentEffectData effects)
        {
 // 集权度由政体成分直接决定（幂等赋值，不累积）            realm.centralization = Mathf.Clamp(effects.centralization, 0f, 1f);
 // 其余瞬时修正（稳定度、税收/军事系数）在此扩展；稳定度类一次性修正禁止在刷新 Tick 中重复叠加。        }

 /// <summary>每 Tick：仅刷新效果缓存；成分被外部路径改变时同步幂等字段</summary>        public void Tick(float deltaTime)
        {
 ...
 /// <summary>重算效果缓存；若成分相对上次发生变化，则同步幂等效果字段（不做自动改制）</summary>        private void RefreshRealmEffects(RealmData realm)
        {
            ...
            if (prev != fp)
            {
 // 成分已被 RegimeChangeDynamics / GovernmentReform 等外部路径改变： // 仅同步幂等字段（centralization），一次性稳定度冲击由落地路径自行记录。                ApplyGovernmentEffects(realm, effects);
                _lastFingerprint[realm.realmId] = fp;
            }
        }

 /// <summary>七维成分指纹（主+次成分），用于检测政体成分是否变化</summary>        private static string Fingerprint(GovernmentComposition c)
        {
 ...
 /// <summary>获取政权的政体效果（缓存）</summary>        public GovernmentEffects.GovernmentEffectData GetRealmEffects(int realmId)
        {
            return _cachedEffects.TryGetValue(realmId, out var effects) ? effects : null;
        }

 /// 手动改变政体（玩家操作 / 事件）。整体替换成分；革新前置应由 UI / GovernmentReform 先校验。        public void ChangeGovernment(int realmId, GovernmentComposition newComposition)
        {
            if (_world == null || _world.realms == null) return;
 ...
 // 政体变化会导致稳定度下降（一次性冲击）            realm.stability = Mathf.Clamp(realm.stability - 20f, 0f, 100f);

            Debug.Log($"[GovernmentSystem] 政权 {realm.realmName} 政体手动变更为: {effects.realmNameSuffix}");
        }

 /// <summary>检查政体效果要求的革新是否已具备（基于效果数据里声明的革新名集合）</summary>        public bool CheckInnovationRequirements(int realmId, HashSet<string> unlockedInnovations)
        {
 ...
 /// <summary>获取政体缺失的革新列表</summary>        public List<string> GetMissingInnovations(int realmId, HashSet<string> unlockedInnovations)
        {
