...
namespace CivilizationEvolution.Politics
{
 ...
 /// 阶层需求维度。每个阶层只关心其中若干维（无关维度权重为 0，不参与归一化）。 /// 维度拆分是为了精确定位"哪个系统导致该阶层不满"，供 UI 提示与 AI 决策。

 /// 政权情境快照：某一时刻从各子系统采集的、与阶层需求相关的客观指标。 /// 全部指标已归一化，ClassNeedsSystem 不关心数据来源。

 /// <summary>单个需求维度的评估结果</summary>

 /// <summary>单个阶层的需求评估报告</summary>

 /// 阶层需求权重表：每阶层对各需求维度的关心权重（内部自动归一化，模组可覆盖）。 /// 权重为 0 表示该阶层基本不关心此维度（如奴隶无政治通道诉求、农民几乎不关心参政）。

 /// 政治通道分析器：解析政体七维成分，判断各阶层在该政体下的政治参与/上升通道畅通度。 /// 这是"上层建筑是否容纳该阶层"的制度性判断——通道被堵的阶层会持续积累政治能量。

 /// 阶层需求系统主体：输入情境快照，输出各阶层多维需求满足度。 /// 无状态、可并行、可单测；权重表可替换以支持模组。    public class ClassNeedsSystem
    {
 // —— 满足度映射系数（集中常量化，便于调参与模组平衡）——        const float SubsistenceBase = 40f, SubsistenceSlope = 50f, SubsistenceDisasterPenalty = 0.2f;
 ...
 /// <summary>维度低于此值视为"主要矛盾"，写入 reason</summary>        public const float GrievanceThreshold = 45f;

        public ClassNeedWeightTable Weights { get; set; } = new ClassNeedWeightTable();

 /// <summary>评估单个阶层的全部需求维度</summary>        public ClassNeedReport EvaluateClass(GameEnums.SocialClass cls, RealmSituation sit)
        {
 ...
 // 加权综合（仅权重>0 的维度参与）            float sumScore = 0f, sumWeight = 0f;
 ...
 // 找最差维度（仅在有权重的维度中）            report.worstScore = 100f;
 ...
 /// <summary>评估政权下全部存在的阶层</summary>        public Dictionary<GameEnums.SocialClass, ClassNeedReport> EvaluateRealm(RealmSituation sit)
        {
 ...
 // ===== 各维度满足度计算（输出 0~100）=====
        ...
        {
 // 奴隶生存线更低、更脆弱            float slope = cls == GameEnums.SocialClass.Slave ? SubsistenceSlope * 0.8f : SubsistenceSlope;
 ...
 // ===== 不满足原因（低于阈值时给出，供 UI/AI）=====        static string ReasonIfLow(float score, string reason) => score < GrievanceThreshold ? reason : "";
