...
{
 /// 继位扶正系统：统治者死亡 → 确定继承人并扶正（君主制=继承法四轴判定， /// 共和制=资格过滤+威望选举）→ 争议判定（绝嗣/幼主）供政体变迁注入    public static class SuccessionSystem
    {


 /// <summary>执行继位（统治者死亡时调用；返回结果供编年史/政体变迁注入）</summary>        public static SuccessionResult ExecuteSuccession(RealmData realm, CharacterManager characters, int day)
        {
 ...
 // 扶正            if (monarchy) realm.monarchId = heir.characterId;
 ...
 /// <summary>候选池：现任统治者的家族在世成员（排除自身）</summary>        private static List<CharacterData> CollectCandidates(RealmData realm, CharacterManager characters, CharacterData ruler)
        {
 ...
 /// <summary>共和制继任：资格过滤（supremeEligibility）后按威望选举</summary>        private static CharacterData SelectByEligibility(RealmData realm, List<CharacterData> candidates)
        {
 ...
 // ===== 大圣战受益人资格（继承法判定） =====
 /// 继承线判定（IsInInheritanceLine——大圣战受益人资格）： /// 线宽 = min(继承顺位候选数, 可分的领地/头衔数) /// 顺位 ≤ 线宽 → 在线内（继承法下会继承——不能当受益人——哪怕他是儿子） /// 顺位 > 线宽 → 线外（轮不到——可以当受益人——哪怕他是儿子/女儿） /// 均分制领地不够分时顺位靠后照样线外（幼子靠圣战挣地=历史常态）        public static bool IsInInheritanceLine(RealmData realm, CharacterManager characters, InheritanceLaw law, int characterId, int divisibleEstates)
        {
            if (realm == null || characters == null || law == null) return false;
            var rulerId = realm.GetSupremeRulerId();
            var ruler = rulerId >= 0 ? characters.GetCharacter(rulerId) : null;

 // 候选池=同 realm 角色（过滤+排序——复用继承法四轴）            var candidates = characters.GetCharactersByRealm(realm.realmId);
 ...
 // 线宽 = min(候选数, 领地可分数)——领地不够分→顺位靠后线外            int lineWidth = Mathf.Min(ordered.Count, divisibleEstates);
            return position < lineWidth;
        }

 /// 大圣战受益人候选（继承线外者——可受益）： /// 候选=同 realm 角色中 IsInInheritanceLine=false 者—— /// 继承与受益制度性分离（圣战土地永不回流家族）        public static List<int> GetGreatHolyWarBeneficiaryCandidates(RealmData realm, CharacterManager characters, InheritanceLaw law, int divisibleEstates)
        {
