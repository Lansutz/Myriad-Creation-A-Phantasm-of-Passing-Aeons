...
namespace CivilizationEvolution.Politics
{
 ...
 /// 派系管理器：从社会画像孕育/更新派系、匹配角色领袖、计算力量、提供博弈查询。 /// 每个政权维护一份派系集合，按政治 Tick 增量更新（不每帧重建，保持派系持续性）。    public class FactionManager
    {
 // realmId → 派系集合        private readonly Dictionary<int, List<Faction>> _factionsByRealm = new Dictionary<int, List<Faction>>();
        private int _nextFactionId = 1;

 // —— 孕育/存续阈值 ——        const float SpawnPowerThreshold = 8f;     // 社会能量达到此值才足以支撑一个派系
        const float DissolvePowerThreshold = 3f;  // 力量长期低于此值则派系消亡
        const float SignificantClassShare = 0.05f;// 阶层人口占比低于 5% 难以独立成派

 /// <summary>获取政权全部派系（只读）</summary>        public IReadOnlyList<Faction> GetFactions(int realmId)
            => _factionsByRealm.TryGetValue(realmId, out var list) ? list : EmptyList;
        static readonly List<Faction> EmptyList = new List<Faction>();

 /// <summary>按立场获取派系</summary>        public Faction GetFactionByStance(int realmId, FactionStance stance)
        {
            if (!_factionsByRealm.TryGetValue(realmId, out var list)) return null;
            return list.FirstOrDefault(f => f.stance == stance);
        }

 /// 按最新社会画像更新政权派系（增量：已存在则更新力量/基础，缺位且社会能量足够则孕育）。 /// characters：本政权有名角色（用于领袖/成员匹配，可空）。        public void UpdateRealmFactions(RealmSociety society, RealmData realm,
 ...
 // 1) 计算四种立场的社会能量与阶层基础            var stanceEnergy = ComputeStanceEnergy(society);

 // 2) 每种立场：存在则更新，缺位且达阈值则创建            foreach (FactionStance stance in Enum.GetValues(typeof(FactionStance)))
            {
 ...
 // 更新阶层基础与政纲                faction.classBacking = energy.backing;
                faction.primaryClass = energy.primaryClass;
                faction.platform = BlendPlatform(faction.classBacking, society, stance);

 // 匹配/更新领袖与成员                MatchCharacters(faction, society, characters);

 // 计算力量                faction.power = ComputePower(faction, energy, characters);
            }

 // 3) 凝聚力漂移 + 消亡清理            for (int i = factions.Count - 1; i >= 0; i--)
            {
                var f = factions[i];
 // 力量与凝聚力正反馈：力量强凝聚力缓升，力量弱凝聚力缓降                f.cohesion = Mathf.Clamp(f.cohesion + (f.power > 20f ? 0.05f : -0.1f), 0f, 100f);
                if (f.power < DissolvePowerThreshold && f.cohesion < 10f)
                    factions.RemoveAt(i);
            }
        }

 // ===== 立场社会能量：把各阶层的 support/unrest 按立场归集 =====        private struct StanceEnergy
        {
 ...
 // 各立场从一个阶层汲取的"能量比例"                    float draw = stance switch
                    {
 // 保守派汲取满意者的支持能量                        FactionStance.Conservative => profile.support / Mathf.Max(1f, profile.influence),
 // 改革派：温和不满且有政治通道的阶层（体制内诉求）                        FactionStance.Reformist => GrievanceBand(profile, 0.15f, 0.55f)
                            * (0.5f + profile.organization),
 // 激进派：深重不满且通道被堵的阶层                        FactionStance.Radical => GrievanceBand(profile, 0.45f, 1f)
 ...
 // 复辟派：在现制度下丧失旧特权的贵族/教士（特权维度严重不满）                        FactionStance.Reactionary => (cls == GameEnums.SocialClass.NobilityClergy || cls == GameEnums.SocialClass.Royalty)
 ...
 /// <summary>不满度落在 [lo,hi] 区间的强度（平滑带），用于区分改革/激进人群</summary>        private static float GrievanceBand(ClassProfile p, float lo, float hi)
        {
            float g = Mathf.Clamp01((100f - p.satisfaction) / 100f);
            if (g <= lo || g >= hi)
            {
 // 软边界：靠近区间仍给少量                float near = Mathf.Max(0f, 0.15f - Mathf.Min(Mathf.Abs(g - lo), Mathf.Abs(g - hi)));
                return near / 0.15f * 0.3f;
            }
            return 1f;
        }

 // ===== 政纲：由阶层基础加权各阶层的典型诉求 =====        private static FactionPlatform BlendPlatform(
            Dictionary<GameEnums.SocialClass, float> backing, RealmSociety society, FactionStance stance)
        {
 // 各阶层的"原生政纲倾向"            var native = new Dictionary<GameEnums.SocialClass, FactionPlatform>
            {
 ...
 // 立场对政纲的整体偏移：保守派维持现状（趋零=不改），激进派放大诉求，复辟派开放度/集权度回摆            float stanceMul = stance switch
            {
 ...
 // ===== 角色匹配：从本政权有名角色中选领袖、吸纳成员 =====        private void MatchCharacters(Faction faction, RealmSociety society, List<CharacterData> characters)
        {
 ...
 // 角色阶层须与派系主要基础相容（backing 显著）                float aff = faction.classBacking.GetValueOrDefault(c.socialClass, 0f);
                if (aff < 0.2f) continue;
                faction.memberCharacterIds.Add(c.characterId);

 // 领袖评分：威望 + 适配六维 + 人格（野心/大胆更愿领头）                float score = c.prestige * 0.5f
 ...
 // ===== 派系力量 = 社会能量 × 凝聚力 × 领袖加成 =====        private static float ComputePower(Faction faction, StanceEnergy energy, List<CharacterData> characters)
        {
            ...
            if (faction.leaderCharacterId >= 0 && characters != null)
            {
                ...
                if (leader != null)
                {
 // 威望与能力提供 0~25% 加成（领袖是放大器，不是力量根源）                    float leaderBonus = (leader.prestige / 1500f) * 0.15f
 ...
 /// <summary>政权内最强派系（关键节点博弈的主导者候选）</summary>        public Faction GetDominantFaction(int realmId)
        {
 ...
 /// <summary>要求"改变现状"的联合力量（改革+激进+复辟）对比"维持现状"力量（保守）</summary>        public void GetChangeVsStatusQuo(int realmId, out float changePower, out float statusQuoPower)
        {
