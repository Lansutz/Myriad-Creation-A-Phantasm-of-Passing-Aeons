...
{
 /// 政权情境采集器：每个政治 Tick 从各子系统采集客观指标，组装成 RealmSituation， /// 供 ClassNeedsSystem 评估。集中在此处对接，避免需求/社会系统直接耦合经济、战争、灾害等。    public static class RealmSituationBuilder
    {
 // 食品货物ID（EconomySystem.ProcessConsumption 以 goodsId=0 为食品）        const int FoodGoodsId = 0;
 // 食品库存"安全线"= 多少天消耗量（库存/日需求 达到此值视为供给充足=1）        const float FoodReserveTargetDays = 30f;

 /// 采集一个政权的需求情境。 /// <param name="realm">政权</param> /// <param name="tiles">全图地块</param> /// <param name="economy">经济管理器（贸易中心库存/商路，可空）</param> /// <param name="wars">进行中的战争列表（可空）</param> /// <param name="armies">全图军队（判断本土交战，可空）</param> /// <param name="disasters">灾害系统（可空）</param> /// <param name="innovations">革新树（阶层制度承认判定，可空=宽松）</param>        public static RealmSituation Build(
 ...
 // ===== 单次遍历政权地块，汇总人口/治安/地区/灾害/占领 =====            float totalPop = 0f, orderSum = 0f, stabilitySum = 0f;
            int landCount = 0;
 ...
 // ===== 粮食保障：本政权涉及地区的食品库存 / (日需求×安全天数) =====            float dailyFoodNeed = Mathf.Max(0.01f, totalPop * 0.01f);
            float foodStock = 0f;
 ...
 // ===== 战争状态 =====            bool atWar = false, enemyOnSoil = occupiedHomeSoil;
            if (wars != null)
            {
 ...
 // ===== 税负痛感（TaxSystem 返回 -50~+10 的影响值，取反为 0~100 痛感）=====            foreach (GameEnums.SocialClass cls in Enum.GetValues(typeof(GameEnums.SocialClass)))
            {
 ...
 // ===== 政治通道（解析政体七维成分）=====            sit.politicalAccess = PoliticalAccessAnalyzer.GetAllAccess(realm.composition);

 // ===== 货币稳定 =====            if (realm.currencySystem != null)
            {
 // 通胀率越高越不稳定；币值（成色/储备）作为补充                float val = realm.currencySystem.GetCurrencyValue();
 ...
 // ===== 阶层制度承认（革新+文化）=====            CultureData culture = null;
 ...
 // ===== 合法性：稳定度 + 威望 =====            sit.stability = realm.stability;
            sit.legitimacy = Mathf.Clamp(realm.stability * 0.6f + realm.prestige * 0.4f, 0f, 100f);

 // ===== 贵族特权保障：世袭最高权力 + 地方世袭领有 + 贵族免税 =====            var comp = realm.composition;
            float priv = 0.3f;
 ...
 /// <summary>枚举政权所有领有地块（核心 + 非核心领有），去重</summary>        static IEnumerable<int> EnumerateRealmTiles(RealmData realm, TileData[] tiles,
            IReadOnlyList<int> realmTiles = null)
        {
 // 优化引（每日一次全扫构建）—— // 避免每政权全扫（N×tiles）——无索引时原逻辑（兼容）            if (realmTiles != null)
            {
