...
{
 /// 政体约束规则系统（条件子选项组设计·完整版） /// 层级结构： /// A. 最高权力 /// A1 交接方式：世袭/选举/推举/僭夺/轮座/神命 /// - 世袭 → 继承法四轴：范围/支系/性别/长幼 /// - 选举 → 选举范围 /// - 推举 → 推举主体 /// - 主导+0~N次要（数量受行政容量限制） /// A2 分配：头衔分配（独享/家族共享）+ 领地分配（独享/均分/采邑） /// B. 中央权力 /// B0 有无：无常设 / 有常设 /// B1 交接方式：任命/选举/考试/世袭 /// B2 机构类型：王庭/议会/长老议事会/官僚中枢/宗教会议/军事委员会 /// - 议会 → 议会构成（一院/两院/等级会议） /// C. 地方权力 /// C1 交接方式：任命/选举/世袭/城市特许（可选项） /// - 任命 → 任命主体 /// - 世袭 → 领有身份 /// - 选举 → 选举范围 /// - 城市特许 → 特许类型 /// C2 职能范围：全权自治/征税司法/仅军事/完全直辖 /// D. 央地结构：单一制/联邦制/邦联制    public static partial class GovernmentConstraints
    {
 // ===== 1. 次要成分限制 =====
 /// 次要成分只适用于最高权力交接（A1） /// 数量受行政容量限制（默认最多2个，行政容量高可更多）        public static bool AllowsSecondary(GovernmentDimension dimension)
        {
            return dimension == GovernmentDimension.SupremeSuccession;
        }

 /// <summary>获取最大次要成分数量（受行政容量影响）</summary>        public static int GetMaxSecondaryCount(float administrativeCapacity = 0.5f)
        {
 // 行政容量0-1，默认0.5对应2个次要 // 低容量(0-0.3): 1个 // 中容量(0.3-0.7): 2个 // 高容量(0.7-1.0): 3个            if (administrativeCapacity < 0.3f) return 1;
            if (administrativeCapacity < 0.7f) return 2;
            return 3;
        }

 /// <summary>政体维度枚举</summary>        public enum GovernmentDimension
        {
 ...
 // ===== 2. 最高权力归属（A0） =====
 /// <summary>最高权力归属：君主制 / 共和制</summary>        public enum SupremeSovereignty
        {
 ...
 // ===== 2. 继承法四轴（世袭君主专用） =====
 /// <summary>继承法范围轴</summary>        public enum InheritanceScope
        {
            ClanOnly,           // 限于本族（同姓宗族）
            BloodRegardless     // 血亲不论姓氏（母系/姻亲也可继承）
        }

 /// <summary>继承法支系轴</summary>        public enum InheritanceBranch
        {
            SeniorLine,         // 长支优先（嫡长支系）
            Fraternal           // 兄终弟及（兄弟相传）
        }

 /// <summary>继承法性别轴</summary>        public enum InheritanceGender
        {
            MaleOnly,           // 仅男性
            MalePreference,     // 男性优先
            Equal,              // 男女平等
 ...
 /// <summary>继承法长幼轴</summary>        public enum InheritanceAge
        {
            ElderFirst,         // 年长者先
            YoungerFirst        // 年幼者先
        }
 ...
 // ===== 3. 最高权力分配（头衔+领地） =====
 /// 最高头衔分配 /// 君主制：必须有最高头衔，独享或家族共享 /// 共和制：最高头衔可选，无则中央机构为主导        public enum TitleDistribution
        {
 ...
 /// <summary>最高领地分配</summary>        public enum DomainDistribution
        {
 ...
 // ===== 4. 中央权力有无 =====
 /// <summary>中央权力有无</summary>        public enum CentralExistence
        {
 ...
 // ===== 5. 条件子选项组定义 =====
 /// <summary>子选项组定义</summary>        public class SubOptionGroup
        {
 ...
 /// <summary>单个子选项</summary>        public class SubOption
        {
            public string name;
            public int value;
 ...
 /// <summary>四轴继承法子选项（世袭君主专用，多轴并行）</summary>        public class InheritanceLawSubOptions
        {
 ...
 // ===== 7. 维度间自由组合（无互斥） =====
        ...
        {
 // 维度之间无互斥，任何组合都有效 // 唯一约束：B0=无常设时，B2必须=None            if (comp.centralExistence == CentralExistence.None &&
 ...
 /// 获取某维度的可用选项（根据当前政体组合过滤） /// 核心规则： /// - A0=共和制 → A1主导成分不能有世袭/神命，但僭夺可作次要成分 /// - A0=君主制 → A2头衔分配只能有独享/家族共享 /// - A0=共和制 → A2头衔分配只能有有头衔/无头衔        public static List<int> GetAvailableOptions(
            GovernmentDimension dimension, GovernmentComposition comp, bool isPrimary = true)
        {
            var allOptions = GetAllOptions(dimension);
            var available = new List<int>();

 // A0最高权力归属：不过滤            if (dimension == GovernmentDimension.SupremeSovereignty)
                return allOptions;

 // A1最高权力交接：根据A0过滤            if (dimension == GovernmentDimension.SupremeSuccession)
            {
                foreach (var option in allOptions)
                {
                    ...
                    {
 // 共和制：排除世袭/神命（僭夺可以有，走主次逻辑）                        if (succession == SupremeSuccession.Hereditary ||
                            succession == SupremeSuccession.Divine)
                            continue;
                    }
 // 君主制：所有选项可用                    available.Add(option);
                }
                return available;
            }

 // B1中央权力交接：共和制下排除世袭            if (dimension == GovernmentDimension.CentralSuccession)
            {
                foreach (var option in allOptions)
                {
                    ...
                    if (comp.supremeSovereignty == SupremeSovereignty.Republic)
                    {
 // 共和制：排除世袭                        if (succession == CentralSuccession.Hereditary)
                            continue;
                    }
 ...
 // C1地方权力交接：共和制下排除世袭            if (dimension == GovernmentDimension.LocalSuccession)
            {
                foreach (var option in allOptions)
                {
                    ...
                    if (comp.supremeSovereignty == SupremeSovereignty.Republic)
                    {
 // 共和制：排除世袭（城市特许可以有，属于自治）                        if (succession == LocalSuccession.Hereditary)
                            continue;
                    }
                    available.Add(option);
                }
 ...
 // 其他维度：不过滤            return allOptions;
        }

 /// 获取头衔分配的可用选项（根据A0最高权力归属过滤） /// 君主制：独享/家族共享 /// 共和制：有最高头衔/无最高头衔        public static List<TitleDistribution> GetAvailableTitleDistributions(GovernmentComposition comp)
        {
