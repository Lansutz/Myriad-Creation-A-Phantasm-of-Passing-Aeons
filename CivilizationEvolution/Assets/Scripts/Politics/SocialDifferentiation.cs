...
namespace CivilizationEvolution.Politics
{
 ...
 /// <summary>社会分化器：无状态静态工具，输入情境快照，就地调整地块人口块的阶层构成</summary>    public static class SocialDifferentiation
    {
 // —— 目标结构常量（集中常量化，便于调参与模组覆盖）——        const float MerchantFloor = 0.02f;       // 工商自由民基础占比（被承认后）
 ...
 /// 对单个政权执行一次社会分化（就地修改其地块人口块）。 /// 应由主循环以固定间隔（如每 20~30 天）调用一次，配合 ConvergeAlpha 实现缓慢演化。        public static void DifferentiateRealm(RealmData realm, TileData[] tiles, RealmSituation sit,
            IReadOnlyList<int> realmTiles = null)
        {
            if (realm == null || tiles == null || sit == null) return;

 // 1) 政权平均发展度（0~1）与各地块列表            float devSum = 0f; int devN = 0;
 ...
 // 2) 政权级目标占比（由制度承认 + 物质条件决定）            var goalShare = ComputeGoalShares(realm, sit, avgDev);

 // 3) 逐地块向目标结构做守恒再平衡（城镇/高发展地块承担更多工商人口）            foreach (int idx in tileList)
            {
                ref TileData t = ref tiles[idx];
 ...
 /// <summary>计算政权级五阶层目标占比（Royalty 不占人口，目标给 0；农民兜底占多数）</summary>        static Dictionary<GameEnums.SocialClass, float> ComputeGoalShares(
 ...
 // 非农总和封顶，保证前现代农业社会以农为主体；超出则等比压缩非农            float nonPeasant = g[GameEnums.SocialClass.MerchantFreeman]
 ...
 /// 单个地块内部的守恒再平衡。高发展度地块的工商目标按局部偏差上调，其余归一。        static void RebalanceTile(ref TileData tile,
 ...
 // 本地块目标占比：工商按"地块发展度/政权平均发展度"偏置（城镇多工商、乡村多农民）            float bias = Mathf.Clamp(tileDev / Mathf.Max(0.05f, avgDev), 0.3f, 2.2f);
 ...
 // 当前各阶层人口            var current = new Dictionary<GameEnums.SocialClass, float>();
 ...
 // 期望净变化；本轮只移动"收缩侧 × alpha"，保证平滑且总量守恒            var delta = new Dictionary<GameEnums.SocialClass, float>();
            float shrinkPool = 0f;
 ...
 // 扩张侧需求总量（用于按比例分配收缩池）            float growNeed = 0f;
            foreach (var kv in delta) if (kv.Value > 0f) growNeed += kv.Value * ConvergeAlpha;
            if (growNeed < 1e-4f) return;

 // 第一步：从收缩阶层释放人口到池（按各阶层收缩缺口比例）            var give = new Dictionary<GameEnums.SocialClass, float>();
 ...
 // 第二步：按扩张需求比例，把池分配给增长阶层            var gain = new Dictionary<GameEnums.SocialClass, float>();
            foreach (var kv in delta)
            {
                if (kv.Value <= 0f) continue;
 ...
 // 执行转移：先从各 give 阶层抽出（可能来自多个块），再补入 gain 阶层            foreach (var gkv in give)
                Withdraw(blocks, gkv.Key, gkv.Value);
            foreach (var rkv in gain)
                if (rkv.Value > 1e-4f)
                    Deposit(blocks, rkv.Key, rkv.Value, blocks);

 // 清除过小碎片块            for (int i = blocks.Count - 1; i >= 0; i--)
                if (blocks[i].count < MinBlockCount) blocks.RemoveAt(i);
        }

 /// <summary>从指定阶层的人口块中抽出 amount（跨多个块），struct 写回</summary>        static void Withdraw(List<PopulationBlock> blocks, GameEnums.SocialClass cls, float amount)
        {
 ...
 /// <summary>把 amount 补入指定阶层；无此阶层块则新建（继承任一现有块的种族/文化/信仰）</summary>        static void Deposit(List<PopulationBlock> blocks, GameEnums.SocialClass cls, float amount,
 ...
 // 新建人口块：继承本地块主体的种族/文化/信仰            var tpl = templateSource.Count > 0 ? templateSource[0] : default;
 ...
 /// <summary>枚举政权所有领有陆地地块（核心 + 非核心领有，去重）</summary>        static IEnumerable<int> EnumerateRealmTiles(RealmData realm, TileData[] tiles,
            IReadOnlyList<int> realmTiles = null)
        {
            if (realmTiles != null)
            {
