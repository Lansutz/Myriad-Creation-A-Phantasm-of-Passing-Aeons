...
{
 /// 大圣战状态（特殊机制——区别于寻常圣战）： /// 号召制（教宗/哈里发号召——该教统/宗教的其他政权参战） /// 战争金库（贡献——简化：参与者列表） /// 土地归属：打下的土地不直接归参战者——结束特殊谈判→所选受益人 /// （受益人=继承法判定继承线外者——继承与受益制度性分离）

 /// <summary>大圣战系统（号召制+受益人谈判——CK3 great_holy_wars.info 同构）</summary>    public static class GreatHolyWarSystem
    {
        private static int _nextWarId = 1;
        public static readonly List<GreatHolyWarState> ActiveWars = new List<GreatHolyWarState>();

 /// 发起大圣战（条件：热忱≥60 + 存在宗教领袖[教宗/哈里发] + /// 目标合理[异教政权/圣地被占]）——号召者=教统领袖所在政权        public static GreatHolyWarState Declare(int faithId, int callerRealmId, int targetRealmId,
 ...
 /// <summary>绑定战争结算（GHW 发起后由 GameWorld 创建 WarState 并绑定—— /// 战争正常分数制结算——结束→圣战方胜→Resolve 受益人谈判）</summary>        public static void BindWar(GreatHolyWarState war, int warId)
        {
            if (war == null) return;
            war.linkedWarId = warId;
        }

 /// <summary>战争结算钩子（GameWorld.UpdateWarOutcomes 后调用： /// 关联战争结束→圣战方胜→Resolve；防御方胜→结束无受益人）</summary>        public static void OnLinkedWarEnded(GreatHolyWarState war, bool attackerWon,
 ...
 /// <summary>号召（该教统/宗教的其他政权响应参战——强制加入[信仰义务]）</summary>        public static void Rally(GreatHolyWarState war, IEnumerable<RealmData> realms)
        {
            if (war == null || war.ended) return;
 ...
 /// 结算（圣战方胜利时）——受益人谈判： /// 候选人=参战政权中继承法线外者（IsInInheritanceLine=false—— /// 次子/幼子/女儿可受益）+ 教会——继承与受益制度性分离 /// 选中受益人 → 目标地块归受益人（新政权/附庸——简化为地块归属）        public static int Resolve(GreatHolyWarState war, CharacterManager characters,
            RealmData callerRealm, int divisibleEstates, InheritanceLaw law)
        {
            if (war == null || war.ended) return -1;
            war.ended = true;

 // 防御方胜利 → 无受益人            if (!war.holySideWon) return -1;

 // 受益人候选=号召者政权中继承线外者（历史：十字军把地给次子/教会）            if (characters == null || callerRealm == null) return -1;
 ...
 /// <summary>清理已结束的战争</summary>        public static void Cleanup()
        {
