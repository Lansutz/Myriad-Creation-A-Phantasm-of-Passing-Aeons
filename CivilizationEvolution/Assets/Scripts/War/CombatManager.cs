...
namespace CivilizationEvolution.War
{
 ...
 /// 解决一场战斗（分阶段推进 + 战斗结束机制）。 /// 设计原则： /// 1. 组织度过低 OR 总伤亡率过高 → 部队暂时失去战斗力 /// 2. 组织度过低 → 增加对方追击效率 /// 3. 瞬时伤亡率高 → 瞬间使组织度下降 /// 4. 总人数不足原有的30% → 战斗结束 /// 5. 逃不逃跑取决于指挥官（能力、士气、训练度）        public BattleResult ResolveBattle(Army attacker, Army defender)
        {
            var result = new BattleResult();

 // 战斗开始：记录原始兵力            attacker.RecordBattleStartManpower(_unitDefs);
 ...
 // ===== 分阶段战斗推进（最多5个阶段，每阶段检查战斗结束）=====            const int MaxPhases = 5;
            ...
            {
 // 每阶段伤亡率（总伤亡率分摊到各阶段）                float phaseFactor = 1f / (MaxPhases - phase); // 后期阶段伤亡更大
 ...
 // 更新伤亡率                attacker.UpdateCasualtyRates(_unitDefs, atkLosses);
                defender.UpdateCasualtyRates(_unitDefs, defLosses);

 // 瞬时伤亡率冲击：瞬间使组织度下降                attacker.ApplyInstantCasualtyOrgShock();
                defender.ApplyInstantCasualtyOrgShock();

 // 组织度自然损耗（训练度越高掉得越慢）                float atkTrainingReduction = 1f - (attacker.training / 100f) * Army.TrainingOrgLossReductionMax;
 ...
 // 检查是否暂时失去战斗力                attacker.CheckCombatIneffective();
                defender.CheckCombatIneffective();

 // 追击效率：如果对方失去战斗力，造成额外伤亡                if (defender.isCombatIneffective)
                {
 ...
 // 检查战斗是否结束（任一方人数不足原有的30%）                if (attacker.CheckBattleEnd(_unitDefs) || defender.CheckBattleEnd(_unitDefs))
                {
                    battleEnded = true;
                    result.battleEndedByManpower = true;
                }

 // 如果败方已失去战斗力且人数不足，战斗结束                if (loser.isCombatIneffective && loser.CheckBattleEnd(_unitDefs))
                {
                    battleEnded = true;
                }
            }

 // ===== 战斗结束：组织度士气变化 + 逃跑判定 =====            if (result.attackerWins)
            {
 ...
 // 败方逃跑判定（取决于指挥官）                result.defenderRetreated = defender.ShouldRetreat();
                if (result.defenderRetreated)
                {
                    defender.state = GameEnums.CombatState.Retreating;
 // 逃跑时检查溃败踩踏事件                    var stampede = StampedeSystem.CheckMilitaryStampede(defender, _tiles, _unitDefs, 50f, 0);
                    if (stampede != null)
                    {
 ...
 // 逃跑时检查溃败踩踏事件                    var stampede = StampedeSystem.CheckMilitaryStampede(attacker, _tiles, _unitDefs, 50f, 0);
                    if (stampede != null)
                    {
 ...
 // ===== 战争闭环（分数累计/白和/胜利判定——接入主循环） =====
 /// <summary>胜利分数阈值（达到即获胜）</summary>        public const float VictoryScore = 100f;

 /// 每日战争推进：同地块敌对军队自动交战（ResolveBattle）， /// 胜方按 WarRules.scoreBattle 加分（×兵力规模系数 0.5~2）        public void DailyTick(Dictionary<int, Army> armies, List<WarState> wars,
            WarRules rules, int day)
        {
            if (wars == null || armies == null) return;

 // 1. 交战检测：遍历战争双方军队，同地块且敌对→战斗            var activeWars = new List<WarState>(wars.FindAll(w => !w.ended));
 ...
 // 找敌方军队（同地块）                    foreach (var otherPair in armies)
                    {
 ...
 // 交战                        var result = ResolveBattle(army, enemy);
 ...
 /// 战争结束判定： /// ①任一方分数 ≥ VictoryScore → 胜利 /// ②双方分数均 < 白和阈值 且 开战超过 peaceMinYears → 白和（allowWhitePeace） /// 返回本日新结束的战争（已标记 ended/winnerId/outcome）        public static List<WarState> UpdateWarOutcomes(List<WarState> wars, WarRules rules, int day)
        {
 ...
 // ① 胜利判定                if (war.attackerScore >= VictoryScore)
                {
 ...
 // ② 白和判定（双方低分 + 开战超年限 + 规则允许）                if (rules != null && rules.allowWhitePeace
