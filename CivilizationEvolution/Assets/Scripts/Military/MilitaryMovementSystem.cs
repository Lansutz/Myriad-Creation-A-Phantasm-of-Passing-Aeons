...
namespace CivilizationEvolution.Military
{
 ...
 /// <summary>敌对堡垒区域通行成本倍率（3级影响）</summary>        public const float EnemyFortCostMultiplier_L3 = 2.0f;

 /// <summary>敌对堡垒区域每Tick损耗率（占军队总人数比例，1级影响）</summary>        public const float EnemyFortLossRate_L1 = 0.005f; // 0.5%
 /// <summary>敌对堡垒区域每Tick损耗率（2级影响）</summary>        public const float EnemyFortLossRate_L2 = 0.012f; // 1.2%
 /// <summary>敌对堡垒区域每Tick损耗率（3级影响）</summary>        public const float EnemyFortLossRate_L3 = 0.025f; // 2.5%

 /// <summary>己方堡垒区域通行成本减成（1级影响）</summary>        public const float FriendlyFortCostReduction_L1 = 0.9f;
 /// <summary>己方堡垒区域通行成本减成（2级影响）</summary>        public const float FriendlyFortCostReduction_L2 = 0.8f;
 /// <summary>己方堡垒区域通行成本减成（3级影响）</summary>        public const float FriendlyFortCostReduction_L3 = 0.7f;

 /// <summary>己方堡垒区域补给率（每Tick恢复比例，1级影响）</summary>        public const float FriendlyFortSupplyRate_L1 = 0.003f; // 0.3%
 /// <summary>己方堡垒区域补给率（2级影响）</summary>        public const float FriendlyFortSupplyRate_L2 = 0.008f; // 0.8%
 /// <summary>己方堡垒区域补给率（3级影响）</summary>        public const float FriendlyFortSupplyRate_L3 = 0.015f; // 1.5%

 /// <summary>己方堡垒区域士气/组织度恢复加成</summary>        public const float FriendlyFortMoraleBonus = 0.1f;
 ...
 // 中立/无主关隘：可通行                if (tile.barrierOwnerRealmId == -1) return true;

 // 敌对关隘且处于战争状态：不可通行（除非攻破）                if (isAtWarWithBarrierOwner) return false;

 // 非战争状态的敌对关隘：可通行（但有关税/检查）                return true;
            }

            return true;
        }

 /// 检查是否需要攻破关隘才能通过        public static bool RequiresBarrierBreach(TileData tile, int armyRealmId)
        {
 ...
 // 堡垒区域影响            if (tile.fortInfluenceLevel > 0 && fortOwnerRealmId >= 0)
            {
                if (fortOwnerRealmId == armyRealmId)
                {
 // 己方堡垒：通行成本减成                    cost *= tile.fortInfluenceLevel switch
                    {
                ...
                {
 // 敌对堡垒：通行成本加成                    cost *= tile.fortInfluenceLevel switch
                    {
 ...
 // 关隘：己方关隘略微降低成本（有维护的通道）            if (tile.hasBarrier && tile.barrierOwnerRealmId == armyRealmId)
                cost *= 0.95f;

            return Mathf.Max(0.2f, cost);
        }

 /// 计算军队通过某地块的实际通行成本（含通行管制） /// 综合：基础地形成本、堡垒区域影响、通行管制（外交）        public static float CalculateActualMovementCostWithControl(TileData tile, int armyRealmId,
 ...
 // 通行管制加成            if (ownerRealm != null && ownerRealm.realmId != armyRealmId)
            {
 ...
 // 通行管制检查            return MovementControlSystem.CanMilitaryPass(tile, armyRealmId, ownerRealm, isAtWar);
        }

 // ===== 军队损耗计算 =====
 /// 计算军队经过某地块时的人员损耗 /// 主要来自敌对堡垒区域的骚扰/劫掠/箭雨 /// <param name="tile">地块数据</param> /// <param name="armySize">军队总人数</param> /// <param name="armyRealmId">军队所属政权ID</param> /// <param name="fortOwnerRealmId">堡垒所有者政权ID</param> /// <param name="ticks">经过该地块的Tick数</param> /// <returns>损耗人数</returns>        public static float CalculateArmyLosses(TileData tile, float armySize,
 ...
 /// 计算军队经过敌对堡垒区域的组织度/士气损耗        public static float CalculateMoraleLoss(TileData tile, int armyRealmId, int fortOwnerRealmId)
        {
            if (tile.fortInfluenceLevel <= 0 || fortOwnerRealmId < 0) return 0f;
 ...
 // ===== 己方堡垒补给支援 =====
 /// 计算己方堡垒区域的补给恢复量        public static float CalculateSupplyRecovery(TileData tile, float armySize,
            int armyRealmId, int fortOwnerRealmId, int ticks = 1)
        {
            if (tile.fortInfluenceLevel <= 0 || fortOwnerRealmId < 0) return 0f;
 ...
 /// 计算己方堡垒区域的组织度/士气恢复加成        public static float CalculateMoraleRecovery(TileData tile, int armyRealmId, int fortOwnerRealmId)
        {
            if (tile.fortInfluenceLevel <= 0 || fortOwnerRealmId < 0) return 0f;
            if (fortOwnerRealmId != armyRealmId) return 0f;
 ...
 /// 计算己方堡垒区域的战斗支援加成（防御时）        public static float CalculateCombatSupportBonus(TileData tile, int armyRealmId, int fortOwnerRealmId)
        {
            if (tile.fortInfluenceLevel <= 0 || fortOwnerRealmId < 0) return 0f;
            if (fortOwnerRealmId != armyRealmId) return 0f;
 ...
 // ===== 关隘战斗 =====
 /// 计算关隘守方防御加成        public static float GetBarrierDefenseBonus(TileData tile)
        {
            if (!tile.hasBarrier) return 0f;
            return BarrierDefenseBonus * (tile.barrierStrength / 5f); // 强度5=30%加成
        }

 /// 计算攻关隘的难度（所需兵力/时间）        public static float CalculateBarrierBreachDifficulty(TileData tile)
        {
            if (!tile.hasBarrier) return 0f;
 ...
 // ===== 军队行进速度 =====
 /// 计算军队在某地块的行进速度（相对于正常速度的比例）        public static float CalculateMovementSpeed(TileData tile, int armyRealmId, int fortOwnerRealmId)
        {
            float cost = CalculateActualMovementCost(tile, armyRealmId, fortOwnerRealmId);
 ...
 /// 获取完整通行状态描述（含管制，用于UI显示）        public static string GetFullMovementStatus(TileData tile, int armyRealmId,
            int fortOwnerRealmId, RealmData ownerRealm, bool isAtWar)
        {
            var status = new List<string>();

 // 基础状态            string baseStatus = GetMovementStatus(tile, armyRealmId, fortOwnerRealmId);
            if (baseStatus != "正常通行") status.Add(baseStatus);

 // 管制状态            if (ownerRealm != null && ownerRealm.realmId != armyRealmId)
            {
