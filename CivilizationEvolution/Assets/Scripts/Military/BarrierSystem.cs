...
namespace CivilizationEvolution.Military
{
 ...
 /// <summary>中等通行成本坡度阈值（度）</summary>        public const float MediumCostSlope = 15f;

 /// <summary>高海拔阈值（雪线/高原，通行成本增加）</summary>        public const float HighElevation = 0.7f;
 ...
 // 综合海拔+坡度决定可通行性 // 只有高海拔(>0.85)且极陡峭(>50°)才不可通行（高山绝壁/雪线以上陡峭山峰） // 山脉中间的山口（高海拔但坡度较低）可以通行 // 低海拔的陡坡（峡谷峭壁）一般也有小径可通行                tile.passable = !(tile.elevation01 >= ImpassableElevation &&
                                  tile.slopeDegree >= ImpassableSlope);

 // 基础通行成本（平原=1.0）                tile.movementCost = CalculateBaseMovementCost(tile);
            }
        }
 ...
 // ===== 坡度加成 =====            if (tile.slopeDegree >= HighCostSlope)
                cost *= 3.0f;
 ...
 // ===== 海拔加成（高海拔缺氧/严寒，通行成本增加）=====            if (tile.elevation01 >= HighElevation)
                cost *= 2.0f; // 雪线/高原
            else if (tile.elevation01 >= MediumElevation)
                cost *= 1.4f; // 山地

 // ===== 海拔×坡度交互（山脉山口：高海拔但坡度适中，成本低于陡峭低海拔）===== // 高海拔+低坡度 = 高原/山口，比高海拔+高坡度容易通行            if (tile.elevation01 >= HighElevation && tile.slopeDegree < MediumCostSlope)
                cost *= 0.7f; // 高原/山口减免

 // 中海拔+高坡度 = 陡峭山地，但比高海拔+高坡度容易            if (tile.elevation01 >= MediumElevation && tile.elevation01 < HighElevation &&
                tile.slopeDegree >= HighCostSlope)
                cost *= 0.85f;

 // ===== 道路减成 =====            cost *= tile.roadLevel switch
            {
 ...
 // ===== 水文加成 ===== // 河流渡口（无桥的河流需要渡河）            if (tile.isRiver && tile.roadLevel == GameEnums.RoadLevel.None)
                cost *= 1.5f;

 // 海岸滩涂            if (tile.isCoast && tile.elevation01 < 0.3f)
                cost *= 1.2f;
 ...
 // ===== 狭窄通道检测 =====
 /// 检测地图上的狭窄通道（可建关隘的位置） /// 狭窄通道定义：可通行地块两侧被不可通行/高成本地块包围        public static List<int> DetectNarrowPassages(TileData[] tiles, int width, int height)
        {
 ...
 // ===== 关隘建造 =====
 /// 检查是否可在某地块建关隘 /// 条件：必须是狭窄通道+可通行+陆地        public static bool CanBuildBarrier(TileData[] tiles, int width, int height, int index)
        {
 ...
 /// 建造关隘        public static bool BuildBarrier(TileData[] tiles, int index, int realmId, float strength = 5f)
        {
            ref TileData tile = ref tiles[index];
 ...
 // 关隘会略微降低通行成本（有关隘=有维护的通道）            tile.movementCost *= 0.9f;

            return true;
        }

 /// 攻破关隘        public static bool BreachBarrier(TileData[] tiles, int index, int attackerRealmId)
        {
            ref TileData tile = ref tiles[index];
            if (!tile.hasBarrier) return false;
 ...
 // 攻破后通行成本上升（关隘被破坏）            tile.movementCost *= 1.3f;

            return true;
        }

 /// 占领关隘（不破坏，直接变更所有者）        public static bool CaptureBarrier(TileData[] tiles, int index, int newRealmId)
        {
            ref TileData tile = ref tiles[index];
            if (!tile.hasBarrier) return false;
 ...
 // ===== 堡垒区域影响计算 =====
 /// 计算所有堡垒的区域影响 /// 为每个地块设置nearbyFortId和fortInfluenceLevel /// 敌对堡垒：经过时损耗+速度下降 /// 己方堡垒：补给+支援加成        public static void CalculateFortInfluence(TileData[] tiles, int width, int height,
            Dictionary<int, BurgData> burgs)
        {
 // 先重置所有地块的堡垒影响            for (int i = 0; i < tiles.Length; i++)
            {
                tiles[i].nearbyFortId = -1;
                tiles[i].fortInfluenceLevel = 0;
            }

 // 遍历所有堡垒类型的Burg            foreach (var burg in burgs.Values)
            {
 ...
 // 堡垒等级决定影响半径和强度                int radius = FortBaseRadius + (int)burg.settlementLevel;
                int influenceLevel = Mathf.Clamp(1 + (int)burg.settlementLevel / 2, 1, 3);

 // 为半径内的地块设置影响（取最近的堡垒）                for (int dy = -radius; dy <= radius; dy++)
                {
 ...
 // 距离衰减                        float dist = Mathf.Sqrt(dx * dx + dy * dy);
                        int level = dist <= radius * 0.5f ? influenceLevel :
                                    dist <= radius * 0.8f ? Mathf.Max(1, influenceLevel - 1) : 1;

 // 只设置最近的堡垒（或更强的堡垒）                        if (tiles[ti].fortInfluenceLevel < level || tiles[ti].nearbyFortId == -1)
                        {
 ...
 /// 获取某地块的堡垒所有者政权ID        public static int GetFortOwnerRealmId(TileData[] tiles, Dictionary<int, BurgData> burgs, int tileIndex)
        {
