...
namespace CivilizationEvolution.Character
{
 ...
 // 家族核心成员        public List<int> memberIds = new List<int>();

 // 递归子家族（分支）        public List<FamilyNode> branches = new List<FamilyNode>();

 // 父家族（null表示主家族）        [NonSerialized] public FamilyNode parentFamily;

 // 家族属性        public float familyPrestige = 0f;
        public float familyWealth = 0f;
        [NonSerialized] public Dictionary<string, float> familyTraditions = new Dictionary<string, float>();


 /// <summary>家族所属政权（-1=未知；家族传统解锁前置革新按此政权检查）</summary>        public int holderRealmId = -1;

 /// <summary>家族故国（homeland——发源地政权；借鉴《地图上发生的事》homeland_country）</summary>        public int homelandCountryId = -1;

 /// <summary>代数标记（generation_marks——每代的命名/标记序列，本地化键）</summary>        public List<string> generationMarks = new List<string>();

 /// <summary>革新树引用（CreateFamily 时由管理器注入；家族传统解锁前置检查用，不入档）</summary>        [NonSerialized] public InnovationTree Innovations;

 // 家徽/纹章        public string coaPattern;
        public string coaColors;

 /// <summary>添加成员</summary>        public void AddMember(int characterId)
        {
            if (!memberIds.Contains(characterId))
                memberIds.Add(characterId);
        }

 /// <summary>创建分支家族</summary>        public FamilyNode CreateBranch(string branchName, int founderId, int year)
        {
 ...
 /// <summary>获取全家族成员（包括所有分支）</summary>        public List<int> GetAllMembers()
        {
 ...
 /// <summary>获取家族总人数</summary>        public int GetTotalMemberCount()
        {
 ...
 /// <summary>获取家族代数（深度）</summary>        public int GetGenerationDepth()
        {
 ...
 /// <summary>查找角色所在的家族节点</summary>        public FamilyNode FindCharacterFamily(int characterId)
        {
 ...
 /// <summary>计算家族总威望</summary>        public float CalculateTotalPrestige()
        {
 ...
 /// 添加家族传统： /// 注册表未定义 → 拒绝并警告；与已传承传统互斥（incompatibleWith）→ 拒绝； /// 解锁前置革新未全部持有（requiredInnovations）→ 拒绝（革新树未注入时跳过检查）； /// 传承强度起点 1（代际深度由家族系统后续累积）        public bool AddFamilyTradition(string traditionId)
        {
 ...
 // 解锁前置革新检查（革新树注入且家族归属政权已知时生效；否则宽松跳过）            if (Innovations != null && holderRealmId >= 0
 ...
 /// <summary>移除家族传统</summary>        public bool RemoveFamilyTradition(string traditionId) => familyTraditions.Remove(traditionId);

 /// <summary>计算家族传统在指定键上的总效果（键由 FamilyTraditionDef.effects 解释，如 unity/prestige/learning）</summary>        public float GetTraditionEffect(string key)
        {
            float total = 0f;
