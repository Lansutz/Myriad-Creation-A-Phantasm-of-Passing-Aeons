...
namespace CivilizationEvolution.Character
{
    ...
    {
 /// <summary>七维固定顺序（遍历、数组下标、模板 bias 对齐均以此为唯一来源）</summary>        public static readonly PersonalityDimension[] All =
        {
 ...
 /// <summary>数据/存档/事件 JSON 使用的字符串键（与历史拼写完全一致，保证旧数据兼容）</summary>        public static string Key(this PersonalityDimension dim) => dim switch
        {
 ...
 /// <summary>中文显示名</summary>        public static string DisplayName(this PersonalityDimension dim) => dim switch
        {
 ...
 /// <summary>字符串键解析为枚举（容错：无法识别返回 false，供数据驱动/事件入口使用）</summary>        public static bool TryParse(string key, out PersonalityDimension dim)
        {
