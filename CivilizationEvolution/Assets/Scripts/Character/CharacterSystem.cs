...
{
 /// 人格七维（企划书 9.3：-100~100，家族遗传基线，压力&gt;60 漂移翻倍）。 /// 定位：底层人格倾向 / AI 行为参数（参考 CK3 ai_boldness / ai_greed / ai_compassion /// / ai_zeal / ai_energy / ai_sociability / ai_honor 的连续值路径）；玩家可见的离散 /// 性格七维映射为五级标签（强负/负/中性/正/强正，每级名词+形容词+行为描述，对齐 CK3 ai_personality_l_*.yml）；统治者额外推导经济原型（见 DetermineEconomicalArchetype）。性格管行为，特质（PersonalityTrait）管能力，二者分离。 /// 统一枚举：取代此前散落于初始化/漂移/亲和/描述/事件各处的 "boldness" 魔法字符串。

 /// <summary>人格七维元数据（唯一权威顺序表 / 字符串键 / 中文名；新增维度只需改此处）</summary>
 /// 统治者经济原型（对齐 CK3 economical_archetype：由性格七维组合推导的互斥行为原型，决定 AI 预算与战略倾向）。 /// CK3 原版 6 原型 + Balanced；扩展 4 原型（Administrator/Schemer/CulturalPatron/GodlessReformer）。 /// 推导优先级见 CharacterData.DetermineEconomicalArchetype。

 /// <summary>经济原型定义（显示名、描述、行为偏置向量）</summary>

 /// <summary>经济原型定义表</summary>

 /// 角色核心数值 /// 普通人口块不存储个体数值，仅有名角色存储完整角色数值

 /// <summary>角色身份</summary>

 /// <summary>统治类型（企划书 9.1：威望/恶名组合）</summary>

 /// 人格特质 /// 三层架构：基础特质 → 复合特质 → 文化特质

 /// 性格标签定义表（对齐 CK3 原版十余个性格特质 × MPD 三级递进）。 /// 每个标签是独立的 PersonalityTrait，直接显示在人物界面；三级之间用 requiredTraits 表示递进（L1→L2→L3）， /// 对立特质用 conflictingTraits 互斥。不搞从七维连续值推导标签的运行时计算。 /// 新增标签只需在此数组追加。

 /// <summary>角色间关系</summary>

 /// 角色间结构性关系（客观身份：血缘/婚姻/师承/上下级——由家族、婚姻、任职派生或显式设定）。 /// 动态情感（朋友/仇敌/恋人等好感状态）不在此枚举：好感高低看 CharacterRelation.opinion 连续值， /// 带机制加成的特殊联结看 BondType；二者分层，勿再在此堆叠 Friend/Rival/Lover/Enemy 等情感标签。

 /// 人物羁绊系统 /// 角色间的特殊关系纽带，提供机制加成

 /// 人物羁绊（后天缔结、提供机制加成的特殊联结——区别于 RelationshipType 的客观身份）。 /// 同一对角色可既有结构关系（如师徒 Mentor/Student）又缔结机制纽带（MentorBond）； /// 动态好感程度由 CharacterRelation.opinion 表达，Bond 只承载结下的"纽带"及其加成。 /// Rivalry（宿怨）与 Nemesis（死敌）为程度不同的敌对纽带，故并存。

 /// 角色管理器 /// 管理所有有名角色、家族、羁绊 /// DNA 系统对接：角色创建时生成/遗传 DNA，表达为初始属性；生育时孟德尔遗传    public partial class CharacterManager
    {
 /// <summary>种族定义表（由 GameWorld 注入，DNA 表达与混血基准依赖）</summary>        public Dictionary<int, RaceData> Races { get; set; }

 /// <summary>革新树（由 GameWorld 注入，家族传统解锁前置检查依赖）</summary>        public InnovationTree Innovations { get; set; }
 /// <summary>经济系统（由 GameWorld 注入，角色饮食联动依赖）</summary>        public EconomyManager Economy { get; set; }
 /// <summary>地块表（由 GameWorld 注入，角色饮食按政权地块定位贸易中心）</summary>        public TileData[] Tiles { get; set; }
 /// <summary>政权表（由 GameWorld 注入，角色饮食/领地定位依赖）</summary>        public Dictionary<int, RealmData> Realms { get; set; }
 ...
 /// <summary>按 id 解析种族（未注入/未找到返回 null）</summary>        private RaceData ResolveRace(int raceId)
        {
            if (Races != null && Races.TryGetValue(raceId, out var race)) return race;
            return null;
        }

 /// <summary>创建新角色</summary> /// <param name="dna">显式 DNA；null 时若有父母则按孟德尔遗传生成，否则按种族基因频率随机</param> /// <param name="fatherId">父角色 id（-1=无）</param> /// <param name="motherId">母角色 id（-1=无）</param> /// <param name="expressionRace">表达基准种族；null 时用 raceId 对应种族（混血场景传双亲基准平均）</param>        public CharacterData CreateCharacter(string firstName, string lastName, int age, bool isMale,
        ...
        {
 // ===== DNA：显式传入 > 父母遗传 > 种族随机 =====            if (dna == null && (fatherId >= 0 || motherId >= 0))
            {
 ...
 // ===== DNA 表达 → 初始属性 =====            RaceData exprRace = expressionRace ?? ResolveRace(raceId);
 ...
 // 个体抗性：种族基准 + DNA 偏移（疾病感染修正用；变革性为种族设定，不做个体级）            character.individualResistance = Mathf.Clamp(
                (exprRace != null ? exprRace.resistanceBaseline : 50f) + expr.resistanceOffset, 0f, 100f);

            if (dna != null)
            {
 // 有 DNA：勇武/学识由种族基准 + DNA 偏移 + 小随机浮动决定，其余四维保持随机                float martialBase = exprRace != null ? exprRace.martialBaseline : 50f;
            ...
            {
 // 无 DNA（兼容旧路径）：全随机                character.martial = UnityEngine.Random.Range(20f, 80f);
                character.diplomacy = UnityEngine.Random.Range(20f, 80f);
                character.stewardship = UnityEngine.Random.Range(20f, 80f);
                character.intrigue = UnityEngine.Random.Range(20f, 80f);
 ...
 // 天赋/缺陷叠加            ApplyTalentDefectEffect(character, expr);

 // ===== 人格七维初始化（企划书 9.3：家族遗传基线 + 随机偏移） =====            InitializePersonality(character, fatherId, motherId);

 // ===== 魅力初始（DNA 外观微调：AA +5 / Aa +2 / aa -3，±10 随机） =====            float appearanceBonus = character.dna != null && character.dna.GetLocus(DnaLocus.Appearance).IsHomozygousDominant ? 5f
 ...
 // ===== 角色模板套用（第九篇角色生成参数模板：年龄范围/六维约束/人格倾向偏移） =====            if (template != null)
                ApplyTemplate(character, template);

 // 身份与社会阶层对齐（修复角色阶层与人口系统断裂）            character.SyncClassFromRole();
            _characters[character.characterId] = character;
            return character;
        }

 /// 套用角色模板（第九篇角色生成参数模板）： /// - 年龄范围：调用方未指定年龄（age<=0）时在模板范围内随机 /// - 六维范围约束：statMin/statMax（0 表示不约束，顺序 martial/diplomacy/warfare/stewardship/intrigue/learning） /// - 人格倾向偏移：七维 bias 叠加（在家族遗传基线之上）        public void ApplyTemplate(CharacterData c, CharacterTemplateDef template)
        {
 ...
 // 人格倾向偏移（七维统一叠加，bias 访问走模板的枚举索引器）            foreach (var pd in PersonalityDimensions.All)
                c.AddPersonality(pd, template.GetPersonalityBias(pd));
        }

 // ===== 人格七维（企划书 9.3：家族遗传基线 + 随机偏移） =====
 /// <summary>人格七维初始化：有父母取双亲平均 ±10（家族遗传基线），无父母围绕 0 随机 ±30</summary>        private void InitializePersonality(CharacterData c, int fatherId, int motherId)
        {
            var father = fatherId >= 0 ? GetCharacter(fatherId) : null;
            var mother = motherId >= 0 ? GetCharacter(motherId) : null;
 ...
 /// <summary>混血基准：双亲种族基准逐项取平均（返回临时 RaceData，仅用于表达）</summary>        private static RaceData AverageRaceBaselines(RaceData a, RaceData b)
        {
            return new RaceData
            {
                raceId = -1,
 ...
 /// <summary>直系血亲（亲子/全同胞）判定</summary>        private static bool IsDirectLineage(CharacterData a, CharacterData b)
        {
 ...
 // 全同胞（同一对父母）            return a.fatherId >= 0 && a.fatherId == b.fatherId
                && a.motherId >= 0 && a.motherId == b.motherId;
        }

 /// <summary>名字生成：文化名字池（type: 0男名 1女名 2姓氏），空池回退文化名</summary>        private static string GenerateName(int cultureId, int type)
        {
 ...
 /// 初始统治者：为每个政权创建统治者+配偶并建家族 /// 角色系统与生育机制的角色源头（政权初始无人治理的填补）        public void CreateInitialRulers(Dictionary<int, RealmData> realms, int currentYear)
        {
 ...
 // 角色模板（注册表有则套用：tmpl_ruler/tmpl_spouse；无则回退随机年龄）                ContentRegistry.TryGetCharacterTemplate("tmpl_ruler", out var rulerTpl);
 ...
 /// <summary>每日角色Tick</summary>        public void DailyTick(int currentDay, int currentYear)
        {
 // 角色更新            foreach (var character in _characters.Values)
            {
                character.DailyTick(currentDay, currentYear);
            }

 // 羁绊更新            foreach (var bond in _bonds)
            {
                bond.DailyTick();
            }

 // 自主生育（最小机制：DNA 遗传持续发生）            AutoProcreate(currentYear);

 // 饮食联动（肥胖驱动）            DailyDiet();

 // 精神疾病触发与缓解            CheckMentalDisorders();

 // 人格亲和漂移（借鉴 MPD 好感机制：性格相投日久生情，相斥渐行渐远）            PersonalityOpinionDrift();

 // 清理死亡角色的军队指挥            foreach (var character in _characters.Values)
            {
 ...
 // ===== 查询接口 =====        public CharacterData GetCharacter(int id)
        {
 ...
 /// <summary>查找政权的现任统治者（Role=Ruler 且存活；无则 null）</summary>        public CharacterData FindRulerOfRealm(int realmId)
        {
            foreach (var c in _characters.Values)
 ...
 /// <summary>寻找最适合的将领</summary>        public CharacterData FindBestGeneral(int realmId)
        {
 ...
 /// <summary>寻找最适合的统治者</summary>        public CharacterData FindBestRuler(int realmId)
        {
            CharacterData best = null;
            float bestScore = 0f;
            foreach (var c in _characters.Values)
