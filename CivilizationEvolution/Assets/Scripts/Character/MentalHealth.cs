...
{
 /// 精神疾病（简单版） /// 角色级状态机，与传染病系统（人口级）分层： /// - 触发：长期高压（压力>80 持续 90 天）、深度恐惧（>80）、高龄心智衰退、重病后谵妄 /// - 效果：属性修正 + 压力恢复速度 + 好感衰减速度 /// - 缓解：压力长期低于 30 可恢复（失智不可逆） /// 数据驱动MentalDisorderDef 由 MentalHealth/MentalHealthDefs.json /// 定义（Base/Mods 可覆盖、模组可新增），id 为字符串；MentalHealthSystem 查询 /// 注册表优先、未初始化回退内置。企划书第九篇无此模块。
 /// <summary>原版内置精神疾病 id 常量（字符串键，模组可新增任意 id）</summary>

 /// <summary>精神疾病定义（数据驱动：定义文件只存键，显示走本地化表）</summary>

 /// <summary>精神疾病系统核心（注册表优先 + 内置回退 + 判定辅助）</summary>    public static class MentalHealthSystem
    {
 // 触发阈值（简单版）        public const int HighStressTriggerDays = 90;   // 压力>80 持续天数 → 抑郁/焦虑
 ...
 /// <summary>原版内置定义表（注册表未初始化/未定义时的回退）</summary>        public static readonly Dictionary<string, MentalDisorderDef> BuiltinDefs =
 ...
 /// <summary>按 id 取定义（注册表优先——模组可覆盖/新增；未初始化回退内置）</summary>        public static MentalDisorderDef GetDef(string id)
        {
 ...
 /// <summary>获取角色精神疾病的显示名（无病返回空串）</summary>        public static string GetDisorderName(CharacterData c)
        {
            var def = GetDef(c.mentalDisorderId);
            return def != null ? def.GetName() : "";
        }

 /// <summary>精神疾病属性修正（六维 + 魅力，作用于显示/判定层）</summary>        public static void ApplyDisorderMods(CharacterData c, out float martial, out float diplomacy,
