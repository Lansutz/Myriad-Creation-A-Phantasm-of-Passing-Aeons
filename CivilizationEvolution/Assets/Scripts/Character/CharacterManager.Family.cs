...
{
 /// CharacterManager.Family —— 家族与生育（生育/婚姻/亲属查询/家族树/自动生育/纽带）（partial class，与 CharacterSystem.cs 共享字段）    public partial class CharacterManager
    {

 // ===== 生育机制（最小实现，DNA 孟德尔遗传入口） =====
 /// 生育：父+母 → 孟德尔遗传 DNA → 后代角色 /// 校验：双方存活、异性、成年（≥16）、非同一人、非直系亲子 /// 近亲允许（文化/法律层面决策），但近亲系数进入遗传（纯合隐性风险上升） /// 混血（父母不同种族）：后代种族取父系，表达基准取双亲种族平均        public CharacterData Procreate(int fatherId, int motherId, int birthYear)
        {
 ...
 // 直系亲子排除            if (IsDirectLineage(father, mother)) return null;

 // 后代身份：父系传承（最小规则）            int childRaceId = father.raceId;
            int childCultureId = father.cultureId;
            int childFaithId = father.faithId;

 // 混血：表达基准取双亲种族平均（基因频率仍用父种族）            var fatherRace = ResolveRace(father.raceId);
 ...
 /// <summary>统计角色的子女人数</summary>        public int CountChildren(int characterId)
        {
            int count = 0;
 ...
 // ===== 婚姻与家族树—家族树与生育衔接） =====
 /// <summary>婚姻：双向设置配偶（异性/成年/存活/非直系——与 Procreate 同检查）</summary>        public bool Marry(int aId, int bId)
        {
 ...
 /// <summary>获取角色配偶（无返回 null）</summary>        public CharacterData GetSpouse(int characterId)
        {
 ...
 /// <summary>获取子女（父或母=指定角色——反查）</summary>        public List<CharacterData> GetChildren(int characterId)
        {
 ...
 /// <summary>获取兄弟姐妹（共享任一父母，排除自身）</summary>        public List<CharacterData> GetSiblings(int characterId)
        {
            var c = GetCharacter(characterId);
            var result = new List<CharacterData>();
 ...
 /// <summary>获取祖先链（父系优先递归，含父母/祖父母…）</summary>        public List<CharacterData> GetAncestors(int characterId, int maxDepth = 4)
        {
            var result = new List<CharacterData>();
            var c = GetCharacter(characterId);
            int depth = 0;
 ...
 /// <summary>获取孙辈及以下（子女的子女——深度 2）</summary>        public List<CharacterData> GetGrandchildren(int characterId)
        {
            var result = new List<CharacterData>();
 ...
 /// <summary>家族树文本（分代缩进：配偶/祖辈/本人/子女/孙辈——家族树面板用）</summary>        public string BuildFamilyTreeText(int characterId)
        {
 ...
 // 父母（双亲显式列出——家族树含父系+母系）            if (c.fatherId >= 0 || c.motherId >= 0)
            {
                sb.AppendLine("  父母：");
                if (c.fatherId >= 0)
                {
 ...
 // 子女            var children = GetChildren(characterId);
            if (children.Count > 0)
            {
 ...
 // 孙辈            var grands = GetGrandchildren(characterId);
            if (grands.Count > 0)
            {
 ...
 /// <summary>自主生育（已婚夫妇优先——配偶生育；未婚保留随机配对，保证 DNA 遗传持续发生）</summary>        private void AutoProcreate(int currentYear)
        {
 ...
 // 已婚夫妇优先生育（spouseId 双向配对——婚姻制度的生育）            foreach (var male in males)
            {
 ...
 // 已婚夫妇每年约 25% 生育概率（≈0.0007/天——比未婚配对高）                if (UnityEngine.Random.value < 0.0007f)
                {
 ...
 // 未婚随机配对（原有机制保留——简化社会未婚生育）            foreach (var male in males)
            {
 ...
 // 每年约 15% 生育概率（≈0.0004/天）                    if (UnityEngine.Random.value < 0.0004f)
                    {
 ...
 /// <summary>创建家族</summary>        public FamilyNode CreateFamily(string familyName, int founderId, int foundingYear, int realmId = -1)
        {
 ...
 /// <summary>创建羁绊</summary>        public CharacterBond CreateBond(int charAId, int charBId, BondType type)
        {
